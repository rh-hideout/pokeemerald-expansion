...
When writing scripts, it is important to use the correct fade macro to avoid odd beahvior from the GBA's behaviors with alpha blending. If the current area is outside (and/or impacted by DNS), use `fadescreenswapbuffers` to fade in and out. Otherwise, it is safe to use `fadescreen`.
