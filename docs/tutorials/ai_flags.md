...
This flag aims to prevent the player from PP stalling the AI by switching between immunities or spamming Protect-like moves. The AI mon's move scores will slowly decay for absorbed moves over time, eventually making its moves unpredictable. More detailed control for this behaviour can be customized in the `ai.h` config file.
