## fixed. See unfixed for still to be fixed:

### tutorial
#### original:
...going to Bella to understand what has to be done next
...before fight with BUM
> ... (unknown) ... (talks about meeting BIRCH)

#### possible solution:
> BELLA: I heard a frightened little girl outside.
> I am quite busy right now preparing food, so 
> can you please see what is happening out there?

### fight with BUM
#### original:
> BUM: First one is always free
> This is not the place to chat, see you later!
> [NAME]: I should go find [CHILD] at ROUTE 103.

go to the LAB
> PROF. BIRCH: [CHILD]? Gone home, I think.
> Or maybe that kid's scrabbling around in tall grass again somewhere...
> If you or your POKéMON get tired, you should get some rest at home.


#### possible solution:
> BUM: First one is always free
> This is not the place to chat, see you later!
> [NAME]: I should finally introduce myself to PROFESSOR BIRCH at his lab.

go to the LAB
> PROF. BIRCH: Oh, hello! You're [NAME], our new next-door neighbor!
> Nice to meet you, at last. My wife already spoke about you.
> Where [CHILD] is, you are asking? Gone home, I think.
> Or maybe that kid's scrabbling around in tall grass again somewhere...
> But don't forget, if you or your POKéMON get tired, you should get some rest at home.

### fight rival on 103
#### original:
Navigate through the tall grass by first walking east and then slightly northwest to find and interact with PROFESSOR BIRCH's child.
> [CHILD]: Okay, so it's this one and that one that live on ROUTE 103...
> Hey, it's [NAME]!
> ...Oh, yeah, Dad gave you a POKéMON.
> Since we're here, how about a little battle?
> I'll teach you what being a TRAINER's about!

Defeat [CHILD] with your Pokèmon.
> [CHILD]: I think I get it. I think I know why my dad has his eye
> out for you now.
> Look, your POKéMON already likes you, even though you just got it.

#### possible solution:
Navigate through the tall grass by first walking east and then slightly northwest to find and interact with PROFESSOR BIRCH's child.
> [CHILD]: Okay, so it's this one and that one that live on ROUTE 103...
> Hey, it's [NAME]!
> What? You've got a POKèMON? From some BUM? ... for free!?
> ...fiddlesticks, this really ruffles my feathers. I've just been looking for one to give you.
> But, since we're here, how about a little battle?
> I'll teach you what being a TRAINER's about!

Defeat [CHILD] with your Pokèmon.
> [CHILD]: Wow... how have you become so strong!
> You must be a very talented trainer!
> Look, your POKéMON already likes you, even though you just got it.

### last talk with BELLA
#### original:
Head back to Neuostheim.
> BELLA: Wait, [NAME]!
> BELLA: [NAME]! [NAME]! Where have you been?
> Oh! What an adorable POKèMON! You got it from a BUM? How strange!
> Here, this was still in one of your moving boxes. 
> They'll put a zip in your step! 
> ...
> BELLA: You're always on the go...
> ...
> Sometime I miss you...
> ...nevermind.
> If you use a PC, you can store items
> The power of science is staggering!

#### possible solution:
...I would cut out the miss you part, they don't know each other for that long.
...maybe integrate with the hint on using PCs

### flag 0 to 1
#### original:
After solving flag 0, entering the DHBW is possible.
> Ahh [NAME], we waited especially for you.
> PROF. FARMER: At the beginning of your studies, each student receives an
> X, which is intended to support you in your learning.
> PROF. FARMER: Today we will visit the museum...
> (PLatzhaltertext)

---end of logical flag 0---
...teleported to the museum
> PROF. FARMER: What's all this mess?

#### possible solution:
After solving flag 0, entering the DHBW is possible.
> Ahh [NAME], we waited especially for you.
> PROF. FARMER: At the beginning of your studies, each student receives an
> X, which is intended to support you in your learning.
> PROF. FARMER: Today we will visit the museum...

> Yesterday, our temporary collection was replaced to items of ancient history.
> Don't ask me why the DHBW needs this right now,
> but why not make this our opportunity to learn something new.
> So folks, let's get going.

---end of logical flag 0---
...teleported to the museum
> PROF. FARMER: What's all this mess?

## unfixed:
Nothing here yet.