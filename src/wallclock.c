...
static void Task_SetClock_HandleInput(u8 taskId)
{
    ...
    else
    {
        ...
        {
            gTasks[taskId].tMoveDir = MOVEMENT_NONE;

            if (JOY_HELD(DPAD_LEFT))
                gTasks[taskId].tMoveDir = MOVEMENT_BACKWARD;

            if (JOY_HELD(DPAD_RIGHT))
                gTasks[taskId].tMoveDir = MOVEMENT_FORWARD;

            if (gTasks[taskId].tMoveDir != MOVEMENT_NONE)
            {
