...
static const u32 sStatEditorBgTiles[] = INCBIN_U32("graphics/ui_menu/background_tileset.4bpp.smol");
static const u32 sStatEditorBgTilemap[] = INCBIN_U32("graphics/ui_menu/background_tileset.bin.smolTM");
...
static void PrintMonStats()
{
    ...
    for(i = 0; i < 6; i++)
    {
        ...
        ConvertIntToDecimalStringN(gStringVar2, currentStat, STR_CONV_MODE_RIGHT_ALIGN, 4);
    ...
    // Calc Totals
    ConvertIntToDecimalStringN(gStringVar2, sStatEditorDataPtr->normalTotal, STR_CONV_MODE_RIGHT_ALIGN, 5);
    AddTextPrinterParameterized4(WINDOW_2, 1, STARTING_X - 5, STARTING_Y + (DISTANCE_BETWEEN_STATS_Y * 6), 0, 0, sMenuWindowFontColors[FONT_WHITE], 0xFF, gStringVar2);
    ...
    ConvertIntToDecimalStringN(gStringVar1, level, STR_CONV_MODE_RIGHT_ALIGN, 4);
    ...
    if (gender != MON_GENDERLESS)
    {
        if (gender == MON_FEMALE)
            ...
            StringCopy(text, gText_MaleSymbol);
        AddTextPrinterParameterized4(WINDOW_3, FONT_NORMAL, 53, 19, 0, 0, sGenderColors[(gender == MON_FEMALE)], TEXT_SKIP_DRAW, text);
    }
...
static u16 GetCurrentEvSingleStatCap(void)
{
    u32 cap = GetCurrentEVCap();
    if (cap > MAX_PER_STAT_EVS)
        cap = MAX_PER_STAT_EVS;
    return (u16)cap;
}

#define EV_MAX_SINGLE_STAT    GetCurrentEvSingleStatCap()
#define EV_MAX_TOTAL          GetCurrentEVCap()
