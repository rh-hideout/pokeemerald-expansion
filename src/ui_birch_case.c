...
static const struct MonChoiceData sStarterChoices[3][9] = {
    // GEN 1 - 3
    {
        ...
        [BALL_TOP_SECOND]       = {SPECIES_CHARMANDER, 5},
        [BALL_TOP_THIRD]        = {SPECIES_SQUIRTLE, 5},

        [BALL_TOP_FOURTH]       = {SPECIES_CHIKORITA, 5},
        [BALL_MIDDLE_FIRST]     = {SPECIES_CYNDAQUIL, 5},
        [BALL_MIDDLE_SECOND]    = {SPECIES_TOTODILE, 5},

        [BALL_MIDDLE_THIRD]     = {SPECIES_TREECKO, 5},
        [BALL_BOTTOM_FIRST]     = {SPECIES_TORCHIC, 5},
        [BALL_BOTTOM_SECOND]    = {SPECIES_MUDKIP, 5},
    },
    // GEN 4 - 6
    {
        [BALL_TOP_FIRST]        = {SPECIES_TURTWIG, 5},
        [BALL_TOP_SECOND]       = {SPECIES_CHIMCHAR, 5},
        ...
        [BALL_BOTTOM_SECOND]    = {SPECIES_FROAKIE, 5},
    },
    // GEN 7 - 9
    {
        [BALL_TOP_FIRST]        = {SPECIES_ROWLET, 5},
        [BALL_TOP_SECOND]       = {SPECIES_LITTEN, 5},
        [BALL_TOP_THIRD]        = {SPECIES_POPPLIO, 5},

        [BALL_TOP_FOURTH]       = {SPECIES_GROOKEY, 5},
        [BALL_MIDDLE_FIRST]     = {SPECIES_SCORBUNNY, 5},
        [BALL_MIDDLE_SECOND]    = {SPECIES_SOBBLE, 5},

        [BALL_MIDDLE_THIRD]     = {SPECIES_SPRIGATITO, 5},
...
static u8 GenerateAbility(void)
{
    return Random() % 3;
}

static u8 GenerateGenderForSpecies(u16 species)
{
...
void InitializeStarterChoices(void)
{
    ...
    for (setIndex = 0; setIndex < 3; setIndex++)
    {
        for (slotIndex = 0; slotIndex < 9; slotIndex++)
        {
            ...
            sCurrentChoices[setIndex][slotIndex].gender = GenerateGenderForSpecies(sCurrentChoices[setIndex][slotIndex].species);
        }
    }
}
...
static void CreateHandSprite()
{
    ...
    for(i=0; i<9; i++)
    {
        if(sCurrentChoices[gSpecialVar_Result][i].species == SPECIES_NONE) // Choose Non Empty Slot To Start In
            continue;
    ...
    SampleUi_DrawMonIcon(sCurrentChoices[gSpecialVar_Result][sBirchCaseDataPtr->handPosition].species);
...
{
    VarSet(VAR_0x8004, 0);
    u16 setIndex = gSpecialVar_Result;
    struct MonChoiceData *choice = &sCurrentChoices[setIndex][sBirchCaseDataPtr->handPosition];
    u16 evs[NUM_STATS] = {0};
    u16 ivs[NUM_STATS] = {0};
    ...
    // Copy EVs/IVs
    for (u8 i = 0; i < NUM_STATS; i++) evs[i] = choice->evs[i];
    for (u8 i = 0; i < NUM_STATS; i++) ivs[i] = choice->ivs[i];

    // Set moves: use defined moves if available, otherwise get level-up moves at the mon's level
    bool8 hasDefinedMoves = FALSE;
    for (u8 i = 0; i < MAX_MON_MOVES; i++)
    {
        if (choice->moves[i] != MOVE_NONE)
        {
            hasDefinedMoves = TRUE;
            break;
        }
    }
    ...
    // Clear flag before giving mon to prevent CreateMon from re-randomizing the species.
    // Restore it afterward for move randomization if needed.
    bool8 wasRandomizeMonForMoves = wasRandomizeMon;
    if (wasRandomizeMon)
        FlagClear(FLAG_RANDOMIZE_MON);
...
static void PrintTextToBottomBar(u8 textId)
{
    ...
    u16 species = sCurrentChoices[gSpecialVar_Result][sBirchCaseDataPtr->handPosition].species;
...
static void Task_BirchCaseMain(u8 taskId)
{
    ...
    if(JOY_NEW(A_BUTTON))
    {
        if(sCurrentChoices[gSpecialVar_Result][sBirchCaseDataPtr->handPosition].species != SPECIES_NONE) // If spot empty don't go to next control flow state
        {
            PlaySE(SE_SELECT);
