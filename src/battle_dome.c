...
static void InitDomeTrainers(void)
{
    ...
    for (i = 0; i < FRONTIER_PARTY_SIZE; i++)
    {
        ...
        monTypesBits |= (1 << (gSpeciesInfo[GetMonData(&gPlayerParty[trainerId], MON_DATA_SPECIES, NULL)].types[0]));
        monTypesBits |= (1 << (gSpeciesInfo[GetMonData(&gPlayerParty[trainerId], MON_DATA_SPECIES, NULL)].types[1]));
    }
    ...
    for (i = 1; i < DOME_TOURNAMENT_TRAINERS_COUNT; i++)
    {
        ...
        for (j = 0; j < FRONTIER_PARTY_SIZE; j++)
        {
            ...
            monTypesBits |= (1 << (gSpeciesInfo[gFacilityTrainerMons[DOME_MONS[i][j]].species].types[0]));
            monTypesBits |= (1 << (gSpeciesInfo[gFacilityTrainerMons[DOME_MONS[i][j]].species].types[1]));
        }
...
static int SelectOpponentMonsFromParty(int *partyMovePoints, bool8 allowRandom)
{
     ...
     && partyMovePoints[0] == partyMovePoints[2])
    {
        if (allowRandom)
        {
            ...
            while (i != DOME_BATTLE_PARTY_SIZE)
            {
                ...
                if (rand != FRONTIER_PARTY_SIZE && !(selectedMonBits & (1 << (rand))))
                {
                    selectedMonBits |= (1 << (rand));
                    i++;
                }
            }
        }
    ...
    else
    {
        ...
        for (i = 0; i < DOME_BATTLE_PARTY_SIZE; i++)
        {
            selectedMonBits |= (1 << (partyPositions[i]));
        }
    }
