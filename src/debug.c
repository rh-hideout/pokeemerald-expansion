...
static void DebugAction_PCBag_Fill_PocketItems(u8 taskId)
{
    ...
    for (itemId = 1; itemId < ITEMS_COUNT; itemId++)
    {
        if (GetPocketForItem(itemId) == POCKET_ITEMS && CheckBagHasSpace(itemId, MAX_BAG_ITEM_CAPACITY))
...
static void DebugAction_PCBag_Fill_PocketKeyItems(u8 taskId)
{
    ...
    for (itemId = 1; itemId < ITEMS_COUNT; itemId++)
    {
        if (GetPocketForItem(itemId) == POCKET_KEY_ITEMS && CheckBagHasSpace(itemId, 1))
            AddBagItem(itemId, 1);
    }
}
