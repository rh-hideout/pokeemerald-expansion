...
static u32 ChooseTypeFilteredWildMonIndex(const struct WildPokemonInfo *encounterTable, enum Type type, bool32 equalizeWeights)
{
    ...
    for (u32 i = 0; i < encounterTable->numSlots; i++)
    {
        ...
        {
            filteredWeight += equalizeWeights ? 1 : slot->weight;
            filteredSlots[filteredCount++] = (struct FilteredSlot) {
                .index = i,
                .threshold = filteredWeight
            };
        }
    }
    ...
    if (filteredWeight > 0)
    {
        u32 val = (Random() * filteredWeight) >> 16;
        u32 index = 0;
        while (val >= filteredSlots[index].threshold) index++;
        ret = filteredSlots[index].index;
    }
    else
    {
        ret = ChooseWildMonIndex(encounterTable);
    }
...
inline u32 ChooseWildMonIndex(const struct WildPokemonInfo *encounterTable)
{
    ...
    s32 val = (Random() * encounterTable->totalWeight) >> 16;
    u32 index = 0;
    while (val > 0) {
        val -= encounterTable->wildPokemon[index].weight;
        index++;
    }
    return index;
}
...
static u16 GenerateFishingWildMon(const struct WildEncounterTypes *encTypes, enum FishingRods rod)
{
...
bool8 DoesCurrentMapHaveFishingMons(enum FishingRods rod)
{
    u32 headerId = GetCurrentMapWildMonHeaderId();
...
void FishingWildEncounter(enum FishingRods rod)
{
    u16 species;
    u32 headerId;
    enum TimeOfDay timeOfDay;
