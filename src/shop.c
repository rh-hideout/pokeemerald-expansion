...
#define SHOP_MENU_PALETTE (gMapHeader.mapLayout->isFrlg ? 11 : 12)
...
static void BuyMenuDecompressBgGraphics(void)
{
    ...
    LoadPalette(gShopMenu_Pal, BG_PLTT_ID(SHOP_MENU_PALETTE), PLTT_SIZE_4BPP);
}
...
static void BuyMenuCopyMenuBgToBg1TilemapBuffer(void)
{
    ...
    {
        if (src[i] != 0)
            dest[i] = src[i] + (0x3E3 | (SHOP_MENU_PALETTE << 12));
    }
}
