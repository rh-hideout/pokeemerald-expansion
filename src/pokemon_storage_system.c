...
static void LoadDisplayMonGfx(u16 species, u32 pid, bool32 isEgg)
{
    ...
    {
        LoadSpecialPokePic(sStorage->tileBuffer, species, pid, TRUE, isEgg);
...
static u16 TryLoadMonIconTiles(u16 species, u32 personality, bool32 isEgg)
{
    ...
    offset = 16 * i;
    species &= GENDER_MASK;
    CpuCopy32(GetMonIconTiles(species, personality, isEgg), (void *)(OBJ_VRAM0) + offset * TILE_SIZE_4BPP, 0x200);
...
static void SetDisplayMonData(void *pokemon, u8 mode)
{
    ...
    else if (mode == MODE_BOX)
    {
        ...
        if (sStorage->displayMonSpecies != SPECIES_NONE)
        {
            ...
            sStorage->displayMonIsEgg = GetBoxMonData(boxMon, MON_DATA_IS_EGG);
            sStorage->displayMonPalette = GetMonSpritePalFromSpeciesAndPersonality(sStorage->displayMonSpecies, isShiny, sStorage->displayMonPersonality, sStorage->displayMonIsEgg);
...
static void MultiMove_SetIconToBg(u8 x, u8 y)
{
    ...
    if (species != SPECIES_NONE)
    {
        const u8 *iconGfx = GetMonIconPtr(species, personality, isEgg);
...
void UpdateSpeciesSpritePSS(struct BoxPokemon *boxMon)
{
    ...
    sStorage->displayMonPalette = GetMonSpritePalFromSpeciesAndPersonality(species, isShiny, pid, isEgg);
