...
static void ShowPostBattleHintText(void)
{
    ...
    while (!i)
    {
        ...
        case HINT_REMAINING_TRAINERS:
            ...
            {
                if ((1 << (i)) & gSaveBlock2Ptr->frontier.pyramidTrainerFlags)
                    textIndex--;
            }
            i = 1;
            break;
...
{
    return gSaveBlock2Ptr->frontier.pyramidTrainerFlags & (1 << (gObjectEvents[eventId].localId - 1));
}
...
static void MarkPyramidTrainerAsBattled(u16 trainerId)
{
    ...
    for (i = 0; i < MAX_PYRAMID_TRAINERS; i++)
    {
        if (gSaveBlock2Ptr->frontier.trainerIds[i] == trainerId)
            gSaveBlock2Ptr->frontier.pyramidTrainerFlags |= (1 << (i));
    }
    ...
    for (i = 0; i < numObjects; i++)
    {
        do
        {
            do
            {
                if (bits & 1)
                {
                    if (!((1 << (squareId)) & gSaveBlock2Ptr->frontier.pyramidRandoms[3]))
                        bits |= 2;
                }
                else
                {
