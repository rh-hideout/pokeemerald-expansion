...
bool8 isMoveAllowed(u16 move) {
    for (u32 i = 0; i < ARRAY_COUNT(RandomizerMovesBans); i++){
        if (move == RandomizerMovesBans[i]){
            return FALSE;
        }
    }
    return TRUE;
...
struct TrainerMon RandomizeTrainerMon(const struct Trainer* trainer, u8 monsCount, u8 slot, u16 baseSeed) {
    ...
    struct TrainerMon mon = trainer->party[slot];
    ...
    u32 seed;
    seed = (u32)baseSeed << 16;
    seed |= (u32)monsCount << 8;
    seed |= slot;

    // Randomize thrown pokeball
    struct Sfc32State ballState = RandomizerRandSeed(RANDOMIZER_REASON_TRAINER_PARTY, seed, mon.ball);
    mon.ball = (u8) RandomizerNextRange(
        &ballState,
        POKEBLOCKS_COUNT + 1
    );
    ...
    // This will benefit from the move randomization feature for that species
    // if this feature is enabled, else, it'll pick from that speice's moveset
    // We create a fake temporary boxMon to get moves from because a TrainerMon don't have a BoxPokemon
    struct BoxPokemon boxMon;

    CreateBoxMon(&boxMon, mon.species, mon.lvl, 0, OTID_STRUCT_PLAYER_ID);

    GiveBoxMonInitialMoveset(&boxMon);

    for (u8 i = 0; i < MAX_MON_MOVES; i++){
        if (mon.moves[i] != MOVE_NONE){
            mon.moves[i] = GetBoxMonData(&boxMon, MON_DATA_MOVE1 + i);
        }
    }
    return mon;
}
