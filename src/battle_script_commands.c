...
static void Cmd_trysetencore(void)
{
    ...
    else
    {
        ...
        {
            if (HasBattlerActedThisTurn(gBattlerTarget))
                gBattleMons[gBattlerTarget].volatiles.encoreTimer = 4; // We need to account for a scenario where Encore changes the move during the same turn.
            else
        ...
        {
            if (HasBattlerActedThisTurn(gBattlerTarget))
                gBattleMons[gBattlerTarget].volatiles.encoreTimer = ((Random() % 5) + 4);
            else
