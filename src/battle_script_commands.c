...
static bool32 AccuracyCalcHelper(u16 move)
{
    ...
    else if ((gStatuses3[gBattlerTarget] & STATUS3_ALWAYS_HITS && gDisableStructs[gBattlerTarget].battlerWithSureHit == gBattlerAttacker)
     || (B_TOXIC_NEVER_MISS >= GEN_6 && gMovesInfo[move].effect == EFFECT_WILL_O_WISP
     && (IS_BATTLER_OF_TYPE(gBattlerAttacker, TYPE_FIRE) || IS_BATTLER_OF_TYPE(gBattlerAttacker, TYPE_GHOST)))
     || gStatuses4[gBattlerTarget] & STATUS4_GLAIVE_RUSH)
    {
        JumpIfMoveFailed(7, move);
        return TRUE;
