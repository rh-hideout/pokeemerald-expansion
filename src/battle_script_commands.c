...
static void Cmd_attackcanceler(void)
{
    ...
    if (CANCELER_MULTIHIT_MOVES < CANCELER_END) // No reason to calc after everything in attackcanceler has been done
    {
    ...
    else if (gBattleStruct->magicCoatActive && !gBattleStruct->bouncedMoveIsUsed)
    {
        ...
        gBattlerTarget = gBattleStruct->moveBouncer;
...
static void AccuracyCheck(bool32 recalcDragonDarts, const u8 *nextInstr, const u8 *failInstr, u16 move)
{
    ...
    if (move == NO_ACC_CALC_CHECK_LOCK_ON || GetMoveEffect(gCurrentMove) == EFFECT_OHKO)
    {
    ...
    else
    {
        ...
        for (battlerDef = 0; battlerDef < gBattlersCount; battlerDef++)
        {
            ...
            if (CanMoveSkipAccuracyCalc(gBattlerAttacker, battlerDef, abilityAtk, abilityDef, move, RUN_SCRIPT))
                continue;
            ...
            if (!RandomPercentage(RNG_ACCURACY, accuracy))
            {
                ...
                if (moveTarget == TARGET_SMART
                    ...
                    && !IsBattlerImmuneToMove(BATTLE_PARTNER(battlerDef)))
                {
...
static void Cmd_settypetorandomresistance(void)
{
    ...
    u32 moveToCheck = gLastResultingMoves[gBattlerTarget];
    u32 typeToCheck = gLastUsedMoveType[gBattlerTarget];
    ...
    else
    {
        if (moveToCheck == MOVE_NONE
         ...
         || moveToCheck == MOVE_STRUGGLE)
        {
            gBattlescriptCurrInstr = cmd->failInstr;
        }
        else if (!BreaksThroughSemiInvulnerablity(gBattlerAttacker, gBattlerTarget, GetBattlerAbility(gBattlerAttacker), GetBattlerAbility(gBattlerTarget), gCurrentMove))
        {
            gBattlescriptCurrInstr = cmd->failInstr;
        }
    ...
    else if (!BreaksThroughSemiInvulnerablity(gBattlerAttacker, gBattlerTarget, GetBattlerAbility(gBattlerAttacker), GetBattlerAbility(gBattlerTarget), moveToCheck))
    {
        gBattlescriptCurrInstr = cmd->failInstr;
    }
    else if (GetActiveGimmick(gBattlerAttacker) == GIMMICK_TERA)
...
static void Cmd_trysetperishsong(void)
{
    ...
    for (i = 0; i < gBattlersCount; i++)
    {
        if (gBattleMons[i].volatiles.perishSong
            || (gBattleStruct->moveResultFlags[i] & MOVE_RESULT_NO_EFFECT) // TODO: Wrong because protect ignored
