...
{
    if (!(gBattleStruct->distortedTypeMatchups & (1 << (battlerDef)))
...
static void Cmd_attackcanceler(void)
{
    ...
    && !(gAbsentBattlerFlags & (1 << (gBattlerTarget)))
...
static void Cmd_ppreduce(void)
{
    ...
    {
        gBattleStruct->distortedTypeMatchups |= (1 << (gBattlerTarget));
...
static void Cmd_adjustdamage(void)
{
    ...
    {
        gBattleStruct->enduredDamage |= (1 << (gBattlerTarget));
        goto END;
    }
...
static void Cmd_multihitresultmessage(void)
{
    ...
    if (gSpecialStatuses[gBattlerTarget].berryReduced
        && !(gMoveResultFlags & MOVE_RESULT_NO_EFFECT))
    {
        gBattleStruct->ateBerry[gBattlerTarget & BIT_SIDE] |= (1 << (gBattlerPartyIndexes[gBattlerTarget]));
...
static void Cmd_tryfaintmon(void)
{
    ...
    else
    {
        ...
        if (!(gAbsentBattlerFlags & (1 << (battler)))
...
static void Cmd_getexp(void)
{
    ...
    case 0: // check if should receive exp at all
        ...
        else
        {
            ...
            gBattleStruct->givenExpMons |= (1 << (gBattlerPartyIndexes[gBattlerFainted]));
        }
        break;
    case 1: // calculate experience points to redistribute
        {
            ...
            for (i = 0; i < PARTY_SIZE; i++)
            {
                if (!((1 << (i)) & sentInBits) && (1 << (i)) & expShareBits)
                    gBattleStruct->expGettersOrder[orderId++] = i;
            }
    ...
    case 2: // set exp value to the poke in expgetter_id and print message
        if (gBattleControllerExecFlags == 0)
        {
            bool32 wasSentOut = ((gBattleStruct->expSentInMons & (1 << (*expMonId))) != 0);
            ...
            else
            {
                ...
                if (IsValidForBattle(&gPlayerParty[*expMonId]))
                {
                    ...
                    if (gBattleTypeFlags & BATTLE_TYPE_DOUBLE)
                    {
                        if (gBattlerPartyIndexes[2] == *expMonId && !(gAbsentBattlerFlags & (1 << (2))))
                            gBattleStruct->expGetterBattlerId = 2;
                        else
                        {
                            if (!(gAbsentBattlerFlags & (1 << (0))))
...
bool32 NoAliveMonsForPlayer(void)
{
    ...
    for (i = 0; i < maxI; i++)
    {
        if (GetMonData(&gPlayerParty[i], MON_DATA_SPECIES) && !GetMonData(&gPlayerParty[i], MON_DATA_IS_EGG)
            && (!(gBattleTypeFlags & BATTLE_TYPE_ARENA) || !(gBattleStruct->arenaLostPlayerMons & (1 << (i)))))
        {
...
static bool32 NoAliveMonsForOpponent(void)
{
    ...
    for (i = 0; i < PARTY_SIZE; i++)
    {
        if (GetMonData(&gEnemyParty[i], MON_DATA_SPECIES) && !GetMonData(&gEnemyParty[i], MON_DATA_IS_EGG)
         && (!(gBattleTypeFlags & BATTLE_TYPE_ARENA) || !(gBattleStruct->arenaLostOpponentMons & (1 << (i)))))
        {
...
static bool32 TryKnockOffBattleScript(u32 battlerDef)
{
        ...
        && !NoAliveMonsForEitherParty())
    {
        ...
        else
        {
            ...
            {
                gWishFutureKnock.knockedOffMons[side] |= (1 << (gBattlerPartyIndexes[battlerDef]));
            }
...
static u32 GetNextTarget(u32 moveTarget, bool32 excludeCurrent)
{
    ...
    for (i = 0; i < MAX_BATTLERS_COUNT; i++)
    {
        if (i != gBattlerAttacker
            ...
            && IsBattlerAlive(i)
            && !(gBattleStruct->targetsDone[gBattlerAttacker] & (1 << (i)))
...
static void Cmd_moveend(void)
{
    ...
    do
    {
        ...
        case MOVEEND_UPDATE_LAST_MOVES:
            ...
            {
                gDisableStructs[gBattlerAttacker].usedMoves |= (1 << (gCurrMovePos));
            ...
            if (!(gAbsentBattlerFlags & (1 << (gBattlerAttacker)))
                && !(gBattleStruct->absentBattlerFlags & (1 << (gBattlerAttacker)))
        ...
        case MOVEEND_NEXT_TARGET: // For moves hitting two opposing Pokemon.
        {
            ...
            gBattleStruct->targetsDone[gBattlerAttacker] |= (1 << (gBattlerTarget));
            if (!(gHitMarker & HITMARKER_UNABLE_TO_USE_MOVE)
                ...
                && !(gHitMarker & HITMARKER_NO_ATTACKSTRING))
            {
                ...
                else if (gBattleStruct->bouncedMoveIsUsed)
                {
                    ...
                    gBattleStruct->targetsDone[gBattlerAttacker] |= (1 << (originalBounceTarget));
        ...
        case MOVEEND_MAGICIAN:
            if (GetBattlerAbility(gBattlerAttacker) == ABILITY_MAGICIAN
              ...
              && !(gWishFutureKnock.knockedOffMons[GetBattlerSide(gBattlerTarget)] & (1 << (gBattlerPartyIndexes[gBattlerTarget])))
              && !DoesSubstituteBlockMove(gBattlerAttacker, gBattlerTarget, gCurrentMove)
              && !(gMoveResultFlags & MOVE_RESULT_NO_EFFECT)
        ...
        case MOVEEND_EJECT_ITEMS:
            {
                ...
                for (i = 0; i < gBattlersCount; i++)
                {
                    ...
                    if (holdEffect == HOLD_EFFECT_EJECT_BUTTON)
                        ejectButtonBattlers |= (1 << (i));
                    else if (holdEffect == HOLD_EFFECT_EJECT_PACK)
                        ejectPackBattlers |= (1 << (i));
                }
                if (ejectButtonBattlers || ejectPackBattlers)
                {
                    ...
                    for (i = 0; i < gBattlersCount; i++)
                    {
                        ...
                        if (battler != gBattlerAttacker && ejectButtonBattlers & (1 << (battler)))
                        {
                        ...
                        else if (ejectPackBattlers & (1 << (battler)))
                        {
                            ...
                            )
                        {
                            ...
                            effect = TRUE;
                            BattleScriptPushCursor();
                            if (ejectButtonBattlers & (1 << (battler)))
                            {
        ...
        case MOVEEND_PICKPOCKET:
            if (IsBattlerAlive(gBattlerAttacker)
              ...
              && !(gWishFutureKnock.knockedOffMons[GetBattlerSide(gBattlerAttacker)] & (1 << (gBattlerPartyIndexes[gBattlerAttacker])))   // But not knocked off
        ...
        case MOVEEND_DANCER: // Special case because it's so annoying
            if (gMovesInfo[gCurrentMove].danceMove)
            {
                ...
                if (!(gBattleStruct->lastMoveFailed & (1 << (gBattlerAttacker))
...
static void Cmd_getswitchedmondata(void)
{
    ...
    BtlController_EmitGetMonData(battler, BUFFER_A, REQUEST_ALL_BATTLE, (1 << (gBattlerPartyIndexes[battler])));
    MarkBattlerForControllerExec(battler);
...
static void Cmd_switchindataupdate(void)
{
    ...
    if (gWishFutureKnock.knockedOffMons[i] & (1 << (gBattlerPartyIndexes[battler])))
    {
        gBattleMons[battler].item = ITEM_NONE;
    }
...
static void ChooseMonToSendOut(u32 battler, u8 slotId)
{
    ...
    gBattleStruct->field_93 &= ~((1 << (battler)));
...
static void Cmd_openpartyscreen(void)
{
    ...
    if (cmd->battler == BS_FAINTED_MULTIPLE_1)
    {
        ...
        else if (gBattleTypeFlags & BATTLE_TYPE_DOUBLE)
        {
            ...
            if ((1 << (2)) & hitmarkerFaintBits && !((1 << (0)) & hitmarkerFaintBits))
            {
                battler = 2;
                if (HasNoMonsToSwitch(battler, PARTY_SIZE, PARTY_SIZE))
                {
            ...
            if ((1 << (3)) & hitmarkerFaintBits && !((1 << (1)) & hitmarkerFaintBits))
            {
                battler = 3;
                if (HasNoMonsToSwitch(battler, PARTY_SIZE, PARTY_SIZE))
                {
    ...
    else if (cmd->battler == BS_FAINTED_MULTIPLE_2)
    {
        if (!(gBattleTypeFlags & BATTLE_TYPE_MULTI))
        {
            if (gBattleTypeFlags & BATTLE_TYPE_DOUBLE)
            {
                ...
                if ((1 << (3)) & hitmarkerFaintBits && hitmarkerFaintBits & (1 << (1)))
                {
                    battler = 3;
        ...
        gBattlerFainted = 0;
        while (!((1 << (gBattlerFainted)) & hitmarkerFaintBits)
        ...
        else
        {
            ...
            *(gBattleStruct->monToSwitchIntoId + battler) = PARTY_SIZE;
            ...
            else
            {
                ...
                if (gAbsentBattlerFlags & (1 << (battlerOpposite)))
...
static void Cmd_switchhandleorder(void)
{
    ...
    case 1:
        ...
        break;
    case 2:
        if (!(gBattleStruct->field_93 & (1 << (battler))))
        {
...
static void UpdateSentMonFlags(u32 battler)
{
    ...
    if (!BattlerHasAi(battler))
        gBattleStruct->appearedInBattle |= (1 << (gBattlerPartyIndexes[battler]));
}
...
static void Cmd_switchineffects(void)
{
    ...
    case BS_FAINTED_MULTIPLE_1: // Saves the battlers.
        ...
        do
        {
            ...
            if (gBattlerFainted >= gBattlersCount)
                break;
            if (gHitMarker & HITMARKER_FAINTED(gBattlerFainted) && !(gAbsentBattlerFlags & (1 << (gBattlerFainted))))
                break;
        } while (1);
...
static bool32 TryCheekPouch(u32 battler, u32 itemId)
{
    if (ItemId_GetPocket(itemId) == POCKET_BERRIES
        ...
        && gBattleStruct->ateBerry[GetBattlerSide(battler)] & (1 << (gBattlerPartyIndexes[battler]))
...
bool32 CanUseLastResort(u8 battler)
{
    ...
    for (i = 0; i < 4; i++)
    {
        if (gBattleMons[battler].moves[i] != MOVE_NONE)
            knownMovesCount++;
        if (i != gCurrMovePos && gDisableStructs[battler].usedMoves & (1 << (i))) // Increment used move count for all moves except current Last Resort.
            usedMovesCount++;
    }
...
static void HandleScriptMegaPrimalBurst(u32 caseId, u32 battler, u32 type)
{
    ...
    if (caseId == 0)
    {
        ...
        BtlController_EmitSetMonData(battler, BUFFER_A, REQUEST_SPECIES_BATTLE, (1 << (gBattlerPartyIndexes[battler])), sizeof(gBattleMons[battler].species), &gBattleMons[battler].species);
        MarkBattlerForControllerExec(battler);
    }
...
static void Cmd_various(void)
{
    ...
    case VARIOUS_SPECTRAL_THIEF:
    {
        ...
        for (i = STAT_ATK; i < NUM_BATTLE_STATS; i++)
        {
            if (gBattleStruct->stolenStats[0] & (1 << (i)))
            {
                gBattleStruct->stolenStats[0] &= ~((1 << (i)));
    ...
    case VARIOUS_PALACE_FLAVOR_TEXT:
    {
        ...
        if (!(gBattleStruct->palaceFlags & (1 << (battler)))
            && gBattleMons[battler].maxHP / 2 >= gBattleMons[battler].hp
            && IsBattlerAlive(battler)
            && !(gBattleMons[battler].status1 & STATUS1_SLEEP))
        {
    ...
    case VARIOUS_ARENA_OPPONENT_MON_LOST:
    {
        ...
        gBattleStruct->arenaLostOpponentMons |= (1 << (gBattlerPartyIndexes[1]));
    ...
    case VARIOUS_ARENA_PLAYER_MON_LOST:
    {
        ...
        gHitMarker |= HITMARKER_PLAYER_FAINTED;
        gBattleStruct->arenaLostPlayerMons |= (1 << (gBattlerPartyIndexes[0]));
    ...
    {
        VARIOUS_ARGS();
        gBattleStruct->alreadyStatusedMoveAttempt |= (1 << (battler));
        break;
    }
    ...
    case VARIOUS_EERIE_SPELL_PP_REDUCE:
    {
        ...
        if (gLastMoves[battler] != 0 && gLastMoves[battler] != 0xFFFF)
        {
            ...
            if (i != MAX_MON_MOVES && gBattleMons[battler].pp[i] != 0)
            {
                ...
                if (!(gDisableStructs[battler].mimickedMoves & (1 << (i)))
                    && !(gBattleMons[battler].status2 & STATUS2_TRANSFORMED))
                {
    ...
    case VARIOUS_TRY_ACTIVATE_BATTLE_BOND:
    {
        ...
        if (gBattleMons[gBattlerAttacker].species == SPECIES_GRENINJA_BATTLE_BOND
            ...
            && !(gBattleStruct->battleBondTransformed[GetBattlerSide(gBattlerAttacker)] & (1 << (gBattlerPartyIndexes[gBattlerAttacker]))))
        {
            gBattleStruct->battleBondTransformed[GetBattlerSide(gBattlerAttacker)] |= (1 << (gBattlerPartyIndexes[gBattlerAttacker]));
            PREPARE_SPECIES_BUFFER(gBattleTextBuff1, gBattleMons[gBattlerAttacker].species);
    ...
    case VARIOUS_CONSUME_BERRY:
    {
        ...
        gBattleStruct->ateBerry[battler & BIT_SIDE] |= (1 << (gBattlerPartyIndexes[battler]));
    ...
    case VARIOUS_TRY_REVIVAL_BLESSING:
    {
        ...
        if (gSelectedMonPartyId != PARTY_SIZE)
        {
            ...
            BtlController_EmitSetMonData(gBattlerAttacker, BUFFER_A, REQUEST_HP_BATTLE, (1 << (gSelectedMonPartyId)), sizeof(hp), &hp);
            MarkBattlerForControllerExec(gBattlerAttacker);
       ...
       )
    {
        ...
        else
        {
            *(gBattleStruct->battlerPartyIndexes + gBattlerTarget) = gBattlerPartyIndexes[gBattlerTarget];
            ...
            gBattleStruct->forcedSwitch |= (1 << (gBattlerTarget));
...
static void Cmd_setfocusenergy(void)
{
    ...
    if ((gMovesInfo[gCurrentMove].effect == EFFECT_DRAGON_CHEER && (!(gBattleTypeFlags & BATTLE_TYPE_DOUBLE) || (gAbsentBattlerFlags & (1 << (battler)))))
...
static void Cmd_mimicattackcopy(void)
{
    ...
    else
    {
        ...
        if (i == MAX_MON_MOVES)
        {
            ...
            gDisableStructs[gBattlerAttacker].mimickedMoves |= (1 << (gCurrMovePos));
            gBattlescriptCurrInstr = cmd->nextInstr;
        }
        else
        {
...
static void Cmd_trychoosesleeptalkmove(void)
{
    ...
    else // at least one move can be chosen
    {
        ...
        } while (((1 << (movePosition)) & unusableMovesBits));
...
static void Cmd_tryspiteppreduce(void)
{
     ...
     && gLastMoves[gBattlerTarget] != MOVE_UNAVAILABLE)
    {
        ...
        if (i != MAX_MON_MOVES && gBattleMons[gBattlerTarget].pp[i] > (B_CAN_SPITE_FAIL >= GEN_4 ? 0 : 1))
        {
            ...
            if (!(gDisableStructs[gBattlerTarget].mimickedMoves & (1 << (i)))
...
static void Cmd_magnitudedamagecalculation(void)
{
    ...
    for (gBattlerTarget = 0; gBattlerTarget < gBattlersCount; gBattlerTarget++)
    {
        if (gBattlerTarget == gBattlerAttacker)
            continue;
        if (!(gAbsentBattlerFlags & (1 << (gBattlerTarget)))) // A valid target was found.
            break;
    }
...
static void Cmd_tryswapitems(void)
{
    ...
    else
    {
        ...
        if (!(gBattleTypeFlags & (BATTLE_TYPE_LINK
                             ...
                             | BATTLE_TYPE_SECRET_BASE
                             | BATTLE_TYPE_RECORDED_LINK))
            && (gWishFutureKnock.knockedOffMons[sideAttacker] & (1 << (gBattlerPartyIndexes[gBattlerAttacker]))
                || gWishFutureKnock.knockedOffMons[sideTarget] & (1 << (gBattlerPartyIndexes[gBattlerTarget]))))
        {
            gBattlescriptCurrInstr = cmd->failInstr;
        }
    ...
    else
    {
        ...
        case ABILITY_NATURAL_CURE:
            ...
            BtlController_EmitSetMonData(battler, BUFFER_A, REQUEST_STATUS_BATTLE,
                                         (1 << (*(gBattleStruct->battlerPartyIndexes + battler))),
