    ...
    if (GetBattlerSide(battlerId) == B_SIDE_PLAYER)
    {
        ...
        else
        {
            for (i = 0, var = PARTY_SIZE - 1, j = 0; j < PARTY_SIZE; j++)
            {
                ...
                else if (gBattleTypeFlags & BATTLE_TYPE_ARENA && gBattleStruct->arenaLostPlayerMons & (1 << (j)))
                {
                    // fainted arena mon
                    gSprites[ballIconSpritesIds[i]].oam.tileNum += 3;
                }
    ...
    else
    {
        ...
        else
        {
            for (var = 0, i = 0, j = 0; j < PARTY_SIZE; j++)
            {
                ...
                else if (gBattleTypeFlags & BATTLE_TYPE_ARENA && gBattleStruct->arenaLostOpponentMons & (1 << (j)))
                {
                     // fainted arena mon
                    gSprites[ballIconSpritesIds[PARTY_SIZE - 1 - var]].oam.tileNum += 3;
                }
...
void CreateAbilityPopUp(u8 battlerId, u32 ability, bool32 isDoubleBattle)
{
    ...
    gBattleStruct->activeAbilityPopUps |= (1 << (battlerId));
...
static void SpriteCb_AbilityPopUp(struct Sprite *sprite)
{
    ...
    else // Hide
    {
        if (sprite->tFrames == 0)
        {
            ...
            {
                gBattleStruct->activeAbilityPopUps &= ~((1 << (sprite->tBattlerId)));
...
{
    if (gBattleStruct->activeAbilityPopUps & (1 << (battlerId)))
    {
