...
static void ExpansionIntro_LoadGraphics(void)
{
    ...
    LoadPalette(sBgPal_Credits, BG_PLTT_ID(0), PLTT_SIZEOF(24));
...
static void SpriteCallback_PorygonHit(struct Sprite* sprite)
{
    ...
    if (sprite->sTimer % 8 == 0)
    {
        if (sprite->sTimer % 16 == 0)
            LoadPalette(sSpritePal_PorygonShiny, PLTT_ID(16 + sprite->oam.paletteNum), PLTT_SIZEOF(8));
        else
            LoadPalette(sSpritePal_Porygon, PLTT_ID(16 + sprite->oam.paletteNum), PLTT_SIZEOF(8));
    }
