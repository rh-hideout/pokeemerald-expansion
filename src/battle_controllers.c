...
void BtlController_HandleGetMonData(u32 battler)
{
    ...
    if (gBattleResources->bufferA[battler][2] == 0)
    {
        ...
        #ifndef NDEBUG
        MgbaPrintf(MGBA_LOG_WARN,"BtlController_HandleGetMonData battler: %d", battler);
        MgbaPrintf(MGBA_LOG_WARN,"BtlController_HandleGetMonData gBattlerPartyIndexes: %d", gBattlerPartyIndexes[battler]);
        #endif
    }
    else
    {
