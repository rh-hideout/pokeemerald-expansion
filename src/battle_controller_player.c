    ...
    else if (JOY_NEW(DPAD_LEFT | DPAD_UP))
    {
        ...
        else
        {
            do
            {
                ...
                if (gAbsentBattlerFlags & (1 << (gMultiUsePlayerCursor))
    ...
    if (JOY_NEW(A_BUTTON) && !gBattleStruct->descriptionSubmenu)
    {
        ...
        case 1:
            ...
            if (moveTarget & (MOVE_TARGET_USER | MOVE_TARGET_USER_OR_SELECTED))
                gMultiUsePlayerCursor = battler;
            else if (gAbsentBattlerFlags & (1 << (GetBattlerAtPosition(B_POSITION_OPPONENT_LEFT))))
...
static void HandleMoveSwitching(u32 battler)
{
    ...
    if (JOY_NEW(A_BUTTON | SELECT_BUTTON))
    {
        ...
        if (gMoveSelectionCursor[battler] != gMultiUsePlayerCursor)
        {
            ...
            if (gDisableStructs[battler].mimickedMoves & (1 << (gMoveSelectionCursor[battler])))
            {
                gDisableStructs[battler].mimickedMoves &= (~(1 << (gMoveSelectionCursor[battler])));
                gDisableStructs[battler].mimickedMoves |= (1 << (gMultiUsePlayerCursor));
            }
...
static void PlayerHandleChooseMove(u32 battler)
{
    ...
    else
    {
        ...
        AssignUsableZMoves(battler, moveInfo->moves);
        gBattleStruct->zmove.viable = (gBattleStruct->zmove.possibleZMoves[battler] & (1 << (gMoveSelectionCursor[battler]))) != 0;
