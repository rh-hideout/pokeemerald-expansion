...
static bool32 IsCurMapPokeCentre(void)
{
    return IsCurMapInLocationList(sSaveLocationPokeCentreList);
}
...
static void TrySetPokeCentreWarpStatus(void)
{
    if (!IsCurMapPokeCentre())
        gSaveBlock2Ptr->specialSaveWarpFlags &= ~POKECENTRE_SAVEWARP;
    else
        gSaveBlock2Ptr->specialSaveWarpFlags |= POKECENTRE_SAVEWARP;
}
