...
static void CreatePartyMonHeldItemSpriteParameterized(u16, enum ItemId, struct PartyMenuBox *);
...
static void ShowOrHideHeldItemSprite(enum ItemId, struct PartyMenuBox *);
...
static bool8 ReturnGiveItemToBagOrPC(enum ItemId);
...
static bool8 DisplayPartyPokemonDataForMoveTutorOrEvolutionItem(u8 slot)
{
    ...
    enum ItemId item = gSpecialVar_ItemId;
...
static void DisplayGaveHeldItemMessage(struct Pokemon *mon, enum ItemId item, bool8 keepOpen, u8 unused)
{
...
static void DisplayTookHeldItemMessage(struct Pokemon *mon, enum ItemId item, bool8 keepOpen)
{
    GetMonNickname(mon, gStringVar1);
    CopyItemName(item, gStringVar2);
...
static void DisplayAlreadyHoldingItemSwitchMessage(struct Pokemon *mon, enum ItemId item, bool8 keepOpen)
{
    GetMonNickname(mon, gStringVar1);
    CopyItemName(item, gStringVar2);
...
static void DisplaySwitchedHeldItemMessage(enum ItemId item, u16 item2, bool8 keepOpen)
{
...
static void GiveItemToMon(struct Pokemon *mon, enum ItemId item)
{
    u8 itemBytes[2];
...
static void CreatePartyMonHeldItemSpriteParameterized(u16 species, enum ItemId item, struct PartyMenuBox *menuBox)
{
    if (species != SPECIES_NONE)
    {
        menuBox->itemSpriteId = CreateSprite(&sSpriteTemplate_HeldItem, menuBox->spriteCoords[2], menuBox->spriteCoords[3], 0);
...
static void ShowOrHideHeldItemSprite(enum ItemId item, struct PartyMenuBox *menuBox)
{
    if (item == ITEM_NONE)
    {
...
static bool8 IsHPRecoveryItem(enum ItemId item)
{
...
static void GetMedicineItemEffectMessage(enum ItemId item, u32 statusCured)
{
...
static bool8 NotUsingHPEVItemOnShedinja(struct Pokemon *mon, enum ItemId item)
{
...
static bool32 IsItemFlute(enum ItemId item)
{
...
u32 ItemIdToBattleMoveId(enum ItemId item)
{
...
u8 GetItemEffectType(enum ItemId item)
{
    u32 statusCure;
...
static bool8 ReturnGiveItemToBagOrPC(enum ItemId item)
{
...
static u8 CheckBattleEntriesAndGetMessage(void)
{
    ...
    for (i = 0; i < maxBattlers - 1; i++)
    {
        ...
        enum ItemId item = GetMonData(&party[order[i] - 1], MON_DATA_HELD_ITEM);
