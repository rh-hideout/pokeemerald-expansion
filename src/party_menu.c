...
static void CB2_ReturnToPartyMenuUsingCandyJar(void);
...
static void CB2_ReturnToPartyMenuWhileLearningMove(void)
{
    ...
    else if (GetItemFieldFunc(gSpecialVar_ItemId) == ItemUseOutOfBattle_CandyJar && gPartyMenu.menuType == PARTY_MENU_TYPE_FIELD && CheckBagHasItem(gSpecialVar_ItemId, 1))
        InitPartyMenu(PARTY_MENU_TYPE_FIELD, PARTY_LAYOUT_SINGLE, PARTY_ACTION_USE_ITEM, TRUE, PARTY_MSG_NONE, Task_ReturnToPartyMenuWhileLearningMove, gPartyMenu.exitCallback);
    else
...
static void PartyMenuTryEvolution(u8 taskId)
{
    ...
    if (targetSpecies != SPECIES_NONE)
    {
        ...
        else if (gSpecialVar_ItemId == ITEM_CANDY_JAR && gPartyMenu.menuType == PARTY_MENU_TYPE_FIELD && CheckBagHasItem(gSpecialVar_ItemId, 1))
            gCB2_AfterEvolution = CB2_ReturnToPartyMenuUsingCandyJar;
        else
            gCB2_AfterEvolution = gPartyMenu.exitCallback;
        BeginEvolutionScene(mon, targetSpecies, canStopEvo, gPartyMenu.slotId);
        DestroyTask(taskId);
...
u8 GetItemEffectType(u16 item)
{
    ...
    else if (itemEffect == NULL && item == ITEM_CANDY_JAR)
