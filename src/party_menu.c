...
static void Task_TryItemUseFormChange(u8 taskId)
{
    ...
    case 1:
        if (gTasks[taskId].tAnimWait == 0)
        {
            ...
            CreatePartyMonIconSpriteParameterized(gTasks[taskId].tTargetSpecies, GetMonData(mon, MON_DATA_PERSONALITY, NULL), FALSE, &sPartyMenuBoxes[gPartyMenu.slotId], 1);
            icon->oam.mosaic = TRUE;
            icon->data[0] = 10;
            icon->data[1] = 1;
            icon->data[2] = taskId;
...
void TryItemHoldFormChange(struct Pokemon *mon, s8 slotId)
{
    if (TryFormChange(mon, FORM_CHANGE_ITEM_HOLD))
    {
        ...
        CreatePartyMonIconSpriteParameterized(species, GetMonData(mon, MON_DATA_PERSONALITY, NULL), FALSE, &sPartyMenuBoxes[slotId], 1);
