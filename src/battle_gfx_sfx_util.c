...
u16 ChooseMoveAndTargetInBattlePalace(u32 battler)
{
    ...
    i = (gBattleStruct->palaceFlags & (1 << (battler))) ? 2 : 0;
    minGroupNum = i;
    ...
    if (chosenMoveId == -1 || chosenMoveId >= MAX_MON_MOVES)
    {
        ...
        if (unusableMovesBits != ALL_MOVES_MASK)
        {
            ...
            for (i = 0; i < MAX_MON_MOVES; i++)
            {
                ...
                if (GetBattlePalaceMoveGroup(battler, moveInfo->moves[i]) == PALACE_MOVE_GROUP_ATTACK && !((1 << (i)) & unusableMovesBits))
                    numMovesPerGroup += (1 << 0);
                if (GetBattlePalaceMoveGroup(battler, moveInfo->moves[i]) == PALACE_MOVE_GROUP_DEFENSE && !((1 << (i)) & unusableMovesBits))
                    numMovesPerGroup += (1 << 4);
                if (GetBattlePalaceMoveGroup(battler, moveInfo->moves[i]) == PALACE_MOVE_GROUP_SUPPORT && !((1 << (i)) & unusableMovesBits))
            ...
            else
            {
                ...
                {
                    i = Random() % MAX_MON_MOVES;
                    if (!((1 << (i)) & unusableMovesBits) && randSelectGroup == GetBattlePalaceMoveGroup(battler, moveInfo->moves[i]))
                        chosenMoveId = i;
                } while (chosenMoveId == -1);
            }
