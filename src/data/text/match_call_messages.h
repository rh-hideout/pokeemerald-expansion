...
const u8 *const gMatchCallFlavourTexts[REMATCH_TABLE_ENTRIES][CHECK_PAGE_ENTRY_COUNT] =
{
    [REMATCH_ROSE] = MCFLAVOUR(AromaLady_Rose),
    [REMATCH_ANDRES] = MCFLAVOUR(RuinManiac_Andres),
    [REMATCH_DUSTY] = MCFLAVOUR(RuinManiac_Dusty),
    [REMATCH_LOLA] = MCFLAVOUR(Tuber_Lola),
    [REMATCH_RICKY] = MCFLAVOUR(Tuber_Ricky),
    [REMATCH_LILA_AND_ROY] = MCFLAVOUR(SisAndBro_LilaAndRoy),
    [REMATCH_CRISTIN] = MCFLAVOUR(Cooltrainer_Cristin),
    [REMATCH_BROOKE] = MCFLAVOUR(Cooltrainer_Brooke),
    [REMATCH_WILTON] = MCFLAVOUR(Cooltrainer_Wilton),
    [REMATCH_VALERIE] = MCFLAVOUR(HexManiac_Valerie),
    [REMATCH_CINDY] = MCFLAVOUR(Lady_Cindy),
    [REMATCH_THALIA] = MCFLAVOUR(Beauty_Thalia),
    [REMATCH_JESSICA] = MCFLAVOUR(Beauty_Jessica),
    [REMATCH_WINSTON] = MCFLAVOUR(RichBoy_Winston),
    [REMATCH_STEVE] = MCFLAVOUR(PokeManiac_Steve),
    [REMATCH_TONY] = MCFLAVOUR(Swimmer_Tony),
    [REMATCH_NOB] = MCFLAVOUR(BlackBelt_Nob),
    [REMATCH_KOJI] = MCFLAVOUR(BlackBelt_Koji),
    [REMATCH_FERNANDO] = MCFLAVOUR(Guitarist_Fernando),
    [REMATCH_DALTON] = MCFLAVOUR(Guitarist_Dalton),
    [REMATCH_BERNIE] = MCFLAVOUR(Kindler_Bernie),
    [REMATCH_ETHAN] = MCFLAVOUR(Camper_Ethan),
    [REMATCH_JOHN_AND_JAY] = MCFLAVOUR(OldCouple_JohnAndJay),
    [REMATCH_JEFFREY] = MCFLAVOUR(BugManiac_Jeffrey),
    [REMATCH_CAMERON] = MCFLAVOUR(Psychic_Cameron),
    [REMATCH_JACKI] = MCFLAVOUR(Psychic_Jacki),
    [REMATCH_WALTER] = MCFLAVOUR(Gentleman_Walter),
    [REMATCH_KAREN] = MCFLAVOUR(SchoolKid_Karen),
    [REMATCH_JERRY] = MCFLAVOUR(SchoolKid_Jerry),
    [REMATCH_ANNA_AND_MEG] = MCFLAVOUR(SrAndJr_AnnaAndMeg),
    [REMATCH_ISABEL] = MCFLAVOUR(Pokefan_Isabel),
    [REMATCH_MIGUEL] = MCFLAVOUR(Pokefan_Miguel),
    [REMATCH_TIMOTHY] = MCFLAVOUR(Expert_Timothy),
    [REMATCH_SHELBY] = MCFLAVOUR(Expert_Shelby),
    [REMATCH_CALVIN] = MCFLAVOUR(Youngster_Calvin),
    [REMATCH_ELLIOT] = MCFLAVOUR(Fisherman_Elliot),
    [REMATCH_ISAIAH] = MCFLAVOUR(Triathlete_Isaiah),
    [REMATCH_MARIA] = MCFLAVOUR(Triathlete_Maria),
    [REMATCH_ABIGAIL] = MCFLAVOUR(Triathlete_Abigail),
    [REMATCH_DYLAN] = MCFLAVOUR(Triathlete_Dylan),
    [REMATCH_KATELYN] = MCFLAVOUR(Triathlete_Katelyn),
    [REMATCH_BENJAMIN] = MCFLAVOUR(Triathlete_Benjamin),
    [REMATCH_PABLO] = MCFLAVOUR(Triathlete_Pablo),
    [REMATCH_NICOLAS] = MCFLAVOUR(DragonTamer_Nicolas),
    [REMATCH_ROBERT] = MCFLAVOUR(BirdKeeper_Robert),
    [REMATCH_LAO] = MCFLAVOUR(NinjaBoy_Lao),
    [REMATCH_CYNDY] = MCFLAVOUR(BattleGirl_Cyndy),
    [REMATCH_MADELINE] = MCFLAVOUR(ParasolLady_Madeline),
    [REMATCH_JENNY] = MCFLAVOUR(Swimmer_Jenny),
    [REMATCH_DIANA] = MCFLAVOUR(Picnicker_Diana),
    [REMATCH_AMY_AND_LIV] = MCFLAVOUR(Twins_AmyAndLiv),
    [REMATCH_ERNEST] = MCFLAVOUR(Sailor_Ernest),
    [REMATCH_CORY] = MCFLAVOUR(Sailor_Cory),
    [REMATCH_EDWIN] = MCFLAVOUR(Collector_Edwin),
    [REMATCH_LYDIA] = MCFLAVOUR(PkmnBreeder_Lydia),
    [REMATCH_ISAAC] = MCFLAVOUR(PkmnBreeder_Isaac),
    [REMATCH_GABRIELLE] = MCFLAVOUR(PkmnBreeder_Gabrielle),
    [REMATCH_CATHERINE] = MCFLAVOUR(PkmnRanger_Catherine),
    [REMATCH_JACKSON] = MCFLAVOUR(PkmnRanger_Jackson),
    [REMATCH_HALEY] = MCFLAVOUR(Lass_Haley),
    [REMATCH_JAMES] = MCFLAVOUR(BugCatcher_James),
    [REMATCH_TRENT] = MCFLAVOUR(Hiker_Trent),
    [REMATCH_SAWYER] = MCFLAVOUR(Hiker_Sawyer),
    [REMATCH_KIRA_AND_DAN] = MCFLAVOUR(YoungCouple_LoisAndHal),
    [REMATCH_WALLY_VR] = MCFLAVOUR(PkmnTrainer_Wally),
    [REMATCH_ROXANNE] = MCFLAVOUR(RockinWhiz_Roxanne),
    [REMATCH_BRAWLY] = MCFLAVOUR(TheBigHit_Brawly),
    [REMATCH_WATTSON] = MCFLAVOUR(SwellShock_Wattson),
    [REMATCH_FLANNERY] = MCFLAVOUR(PassionBurn_Flannery),
    [REMATCH_NORMAN] = MCFLAVOUR(ReliableOne_Dad),
    [REMATCH_WINONA] = MCFLAVOUR(SkyTamer_Winona),
    [REMATCH_TATE_AND_LIZA] = MCFLAVOUR(MysticDuo_TateAndLiza),
    [REMATCH_JUAN] = MCFLAVOUR(DandyCharm_Juan),
    [REMATCH_SIDNEY] = MCFLAVOUR(EliteFour_Sidney),
    [REMATCH_PHOEBE] = MCFLAVOUR(EliteFour_Phoebe),
    [REMATCH_GLACIA] = MCFLAVOUR(EliteFour_Glacia),
    [REMATCH_DRAKE] = MCFLAVOUR(EliteFour_Drake),
    [REMATCH_WALLACE] = MCFLAVOUR(Champion_Wallace),
};
