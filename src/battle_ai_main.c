...
static u32 AI_CalcMoveEffectScore(u32 battlerAtk, u32 battlerDef, u32 move)
{
    ...
    case EFFECT_BESTOW:
        ...
        case HOLD_EFFECT_MOLUGANION:
            if(gBattleMons[gBattlerTarget].species != SPECIES_NOXILIUM)
                ADJUST_SCORE(DECENT_EFFECT);
            break;
        ...
        case HOLD_EFFECT_UTILITY_UMBRELLA:
            ...
            if (aiData->abilities[battlerAtk] != ABILITY_SHOWER_POWER && aiData->abilities[battlerAtk] != ABILITY_DRY_SKIN)
            {
                switch (aiData->abilities[battlerDef])
                {
    ...
    case EFFECT_ENTRAINMENT:
        if (GetActiveGimmick(battlerDef) == GIMMICK_DYNAMAX)
            break;
        if (aiData->abilities[battlerDef] != aiData->abilities[battlerAtk] && !(gStatuses3[battlerDef] & STATUS4_GASTRO_ACID))
        {
        ...
        if (gMovesInfo[move].additionalEffects[i].self)
        {
            ...
            else
            {
                ...
                case MOVE_EFFECT_ATK_DEF_DOWN:
                    ...
                    ADJUST_SCORE(IncreaseStatUpScoreContrary(battlerAtk, battlerDef, STAT_CHANGE_DEF));
                    break;
            case MOVE_EFFECT_EVOCATION:
                    ADJUST_SCORE(IncreaseStatUpScoreContrary(battlerAtk, battlerDef, STAT_CHANGE_SPATK));
                    ADJUST_SCORE(IncreaseStatUpScoreContrary(battlerAtk, battlerDef, STAT_CHANGE_ATK));
                    break;
                ...
                case MOVE_EFFECT_STEAL_ITEM:
                    {
                        ...
                        && aiData->abilities[battlerDef] != ABILITY_STICKY_HOLD)
                        {
                            ...
                            case HOLD_EFFECT_FLAME_ORB:
                                ...
                                break;
                            case HOLD_EFFECT_MOLUGANION:
                                if(gBattleMons[gBattlerAttacker].species == SPECIES_NOXILIUM)
                                    ADJUST_SCORE(DECENT_EFFECT);
                                break;
                            case HOLD_EFFECT_BLACK_SLUDGE:
