...
// the following checks apply to any target (including user)
    ...
    case EFFECT_TAKE_HEART:
        if ((!(gBattleMons[battlerAtk].status1 & STATUS1_ANY)
         || PartnerMoveEffectIs(BATTLE_PARTNER(battlerAtk), aiData->partnerMove, EFFECT_HEAL_PARTY)
         || PartnerMoveEffectIs(BATTLE_PARTNER(battlerAtk), aiData->partnerMove, EFFECT_HEAL_PARTY_STATUS))
...
static s32 AI_CalcMoveEffectScore(enum BattlerId battlerAtk, enum BattlerId battlerDef, enum Move move, struct AiLogicData *aiData)
{
    ...
    case EFFECT_LEECH_SEED:
        if (IS_BATTLER_OF_TYPE(battlerDef, TYPE_GRASS)
          ...
          || HasMoveWithEffect(battlerDef, EFFECT_REMOVE_TRAPS)
    ...
    for (u32 effectId = 0; effectId < additionalEffectCount; effectId++)
    {
        ...
        else // consider move effects that hinder the target
        {
            ...
            case MOVE_EFFECT_WRAP:
                if (!HasMoveWithEffect(battlerDef, EFFECT_REMOVE_TRAPS) && ShouldTrap(battlerAtk, battlerDef, move, CONSIDER_WRAP_DAMAGE))
                    ADJUST_SCORE(BEST_EFFECT);
                break;
