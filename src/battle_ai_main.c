...
// the following checks apply to any target (including user)
    ...
    case EFFECT_DARK_VOID:
        if (B_DARK_VOID_FAIL >= GEN_7 && (gBattleMons[battlerAtk].species != SPECIES_YUBEL 
            && gBattleMons[battlerAtk].species != SPECIES_YUBEL_TERROR_INCARNATE 
            && gBattleMons[battlerAtk].species != SPECIES_YUBEL_ULTIMATE_KNIGHTMARE))
            ADJUST_SCORE(-10);
        break;
    ...
    case MOVE_EFFECT_BURN:
        ...
        break;
    case MOVE_EFFECT_FREEZE:
        if (!AI_CanFreeze(battlerAtk, battlerDef, aiData->abilities[battlerDef], move, aiData->partnerMove))
            ADJUST_SCORE(-10);
        if (!ShouldFreezeOrFrostbite(battlerAtk, battlerDef, aiData->abilities[battlerDef]))
            ADJUST_SCORE(-5);
        break;
    default:
        break;
...
static s32 AI_CalcMoveEffectScore(enum BattlerId battlerAtk, enum BattlerId battlerDef, enum Move move, struct AiLogicData *aiData)
{
    ...
    case EFFECT_DRAGON_SWORD:
        if (gBattleMons[battlerAtk].statStages[STAT_ATK] < MAX_STAT_STAGE
        && IS_BATTLER_OF_TYPE(battlerDef, TYPE_DRAGON)
        && aiData->abilities[battlerAtk] != ABILITY_CONTRARY
        && CanIndexMoveFaintTarget(battlerAtk, battlerDef, movesetIndex, AI_ATTACKING))
            ADJUST_SCORE(BEST_EFFECT);
        break;
