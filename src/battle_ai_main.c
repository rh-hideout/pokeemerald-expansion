...
// the following checks apply to any target (including user)
    ...
    // other
        ...
        case EFFECT_DOODLE: // Same as Role Play, but also check if the partner's ability should be replaced
            ...
            break;
        case EFFECT_WISH:
            if (gBattleStruct->battlerState[battlerAtk].wishCounter > gBattleTurnCounter)
                ADJUST_SCORE(-10);
            break;
        case EFFECT_ASSIST:
            if (CountUsablePartyMons(battlerAtk) == 0)
    ...
    case EFFECT_REST:
        ...
        else if (ShouldRecover(battlerAtk, battlerDef, move, 100, AI_DEFENDING))
        {
            if (aiData->holdEffects[battlerAtk] == HOLD_EFFECT_CURE_SLP
              ...
              || (AI_GetWeather() & B_WEATHER_RAIN && gFieldTimers.weatherDuration != 1 && aiData->abilities[battlerAtk] == ABILITY_HYDRATION && aiData->holdEffects[battlerAtk] != HOLD_EFFECT_UTILITY_UMBRELLA))
                ADJUST_SCORE(GOOD_EFFECT);
        }
        break;
    case EFFECT_OHKO:
