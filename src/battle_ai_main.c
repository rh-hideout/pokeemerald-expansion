...
static s32 AI_DoubleBattle(enum BattlerId battlerAtk, enum BattlerId battlerDef, enum Move move, s32 score)
{
    ...
    case EFFECT_COACHING:
        ...
        else if (HasMoveWithEffect(battlerAtkPartner, EFFECT_STORED_POWER))
        {
            ...
            if (BattlerShouldChangeStats(battlerAtkPartner, BATTLE_OPPOSITE(battlerAtk), move, GetBattlerMoveIndexWithEffect(battlerAtkPartner, EFFECT_STORED_POWER), CHANGE_STAT_DMG_DEALT, partnerStatChanges, defStatChanges)
             || BattlerShouldChangeStats(battlerAtkPartner, BATTLE_OPPOSITE(battlerAtkPartner), move, GetBattlerMoveIndexWithEffect(battlerAtkPartner, EFFECT_STORED_POWER), CHANGE_STAT_DMG_DEALT, partnerStatChanges, defStatChanges))
                ADJUST_SCORE(GOOD_EFFECT);
        }
        else
        {
