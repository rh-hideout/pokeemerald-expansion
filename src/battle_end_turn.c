...
static bool32 HandleEndTurnWeatherDamage(u32 battler)
{
    ...
    if (currBattleWeather == 0xFF)
    {
        ...
        gBattleStruct->state.endTurnBattler = 0;
        gBattleStruct->state.endTurn++;
        return effect;
    }

    gBattleStruct->state.endTurnBattler++;
...
static bool32 HandleEndTurnFirstEventBlock(u32 battler)
{
    ...
    if (!IsBattlerAlive(battler))
    {
        gBattleStruct->state.endTurnBlock = 0;
        gBattleStruct->state.endTurnBattler++;
        return effect;
    }

    switch (gBattleStruct->state.endTurnBlock)
    {
    case FIRST_EVENT_BLOCK_GMAX_MOVE_RESIDUAL: // TODO: Has to be split into 3 statuses and needs a queue
        ...
        gBattleStruct->state.endTurnBlock++;
        break;
...
static bool32 HandleEndTurnSecondEventBlock(u32 battler)
{
    ...
    u32 side = gBattleStruct->state.battlerSide;
    ...
    case SECOND_EVENT_BLOCK_AURORA_VEIL:
        ...
        gBattleStruct->state.battlerSide++;
        gBattleStruct->state.endTurnBlock = 0;
        break;
    }
...
u32 DoEndTurnEffects(void)
{
    ...
    for (;;)
    {
        ...
        if (gBattleStruct->state.endTurnBattler == gBattlersCount || gBattleStruct->state.battlerSide == NUM_BATTLE_SIDES)
        {
            gBattleStruct->state.endTurnBattler = 0;
            gBattleStruct->state.battlerSide = 0;
            gBattleStruct->state.endTurnBlock = 0;
            gBattleStruct->state.endTurn++;
        }

        // Jump out if possible after endTurnEventsCounter was increased in the above code block
        if (gBattleStruct->state.endTurn == ENDTURN_COUNT)
            return FALSE;

        battler = gBattlerAttacker = gBattlerByTurnOrder[gBattleStruct->state.endTurnBattler];

        if (sEndTurnEffectHandlers[gBattleStruct->state.endTurn](battler))
            return TRUE;
    }
}
