...
void BreakStringAutomatic(u8 *src, u32 maxWidth, u32 screenLines, u8 fontId, bool32 showPrompt)
{
    ...
    while (src[currIndex] != EOS)
    {
        if (src[currIndex] == CHAR_PROMPT_CLEAR)
        {
            ...
            src[currIndex + 1] = EOS;
            BreakSubStringAutomatic(currSrc, maxWidth, screenLines, fontId, showPrompt);
...
void BreakSubStringAutomatic(u8 *src, u32 maxWidth, u32 screenLines, u8 fontId, bool32 showPrompt)
{
    ...
    if (totalWidth > maxWidth)
    {
        ...
        BuildNewString(stringLines, totalLines, screenLines, src, showPrompt);
        Free(stringLines);
    }
...
void BuildNewString(struct StringLine *stringLines, u32 numLines, u32 maxLines, u8 *str, u32 showPrompt)
{
    u32 srcCharIndex = 0;
    for (u32 lineIndex = 0; lineIndex < numLines; lineIndex++)
    {
        ...
        if (lineIndex + 1 < numLines)
        {
            ...
            if (lineIndex >= maxLines - 1 && numLines > maxLines && showPrompt)
