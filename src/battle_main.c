...
const struct TrainerClass gTrainerClasses[TRAINER_CLASS_COUNT] =
{
    ...
    [TRAINER_CLASS_KARATE_COUPLE] = { _("CRUSH KIN") },
    ...
    [TRAINER_CLASS_ROCKET_GRUNT] = { _("TEAM ROCKET") },
...
u32 GetBattlerTotalSpeedStat(u32 battler, enum Ability ability, enum HoldEffect holdEffect)
{
    ...
    if (ability == ABILITY_QUICK_FEET && gBattleMons[battler].status1 & STATUS1_ANY)
        speed = (speed * 150) / 100;
    else if (ability == ABILITY_TOXIC_BOOST && gBattleMons[battler].status1 & STATUS1_POISON)
        speed *= 1.5;
    else if (ability == ABILITY_SURGE_SURFER && gFieldStatuses & STATUS_FIELD_ELECTRIC_TERRAIN)
        speed *= 2;
    else if (ability == ABILITY_MISTY_STEP && gFieldStatuses & STATUS_FIELD_MISTY_TERRAIN)
        speed *= 2;
