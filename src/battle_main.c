...
void SwitchInClearSetData(u32 battler)
{
    ...
    gBattleStruct->lastMoveFailed &= ~((1 << (battler)));
    ...
    gBattleStruct->enduredDamage &= ~(1 << (battler));
    ...
    case FIRST_TURN_EVENTS_END:
        ...
        *(&gBattleStruct->absentBattlerFlags) = gAbsentBattlerFlags;
        ...
        for (i = 0; i < gBattlersCount; i++)
        {
            ...
            if (!BattlerHasAi(i))
                gBattleStruct->appearedInBattle |= (1 << (gBattlerPartyIndexes[i]));
        }
...
static void HandleTurnActionSelectionState(void)
{
    ...
    for (battler = 0; battler < gBattlersCount; battler++)
    {
        ...
        case STATE_BEFORE_ACTION_CHOSEN: // Choose an action.
            *(gBattleStruct->monToSwitchIntoId + battler) = PARTY_SIZE;
            if (gBattleTypeFlags & BATTLE_TYPE_MULTI
                ...
                || gBattleStruct->absentBattlerFlags & (1 << (GetBattlerAtPosition(BATTLE_PARTNER(position))))
                || gBattleCommunication[GetBattlerAtPosition(BATTLE_PARTNER(position))] == STATE_WAIT_ACTION_CONFIRMED)
            {
                if (gBattleStruct->absentBattlerFlags & (1 << (battler)))
                {
        ...
        case STATE_WAIT_ACTION_CHOSEN: // Try to perform an action.
            if (!(gBattleControllerExecFlags & (((1 << (battler))) | (0xF << 28) | ((1 << (battler)) << 4) | ((1 << (battler)) << 8) | ((1 << (battler)) << 12))))
            {
                ...
                case B_ACTION_CANCEL_PARTNER:
                    ...
                    gBattleStruct->gimmick.toActivate &= ~((1 << (BATTLE_PARTNER(GetBattlerPosition(battler)))));
                    BtlController_EmitEndBounceEffect(battler, BUFFER_A);
                    MarkBattlerForControllerExec(battler);
                    return;
                case B_ACTION_DEBUG:
                    ...
                    default:
                        ...
                        else
                        {
                            ...
                            if (gBattleResources->bufferB[battler][2] & RET_GIMMICK)
                                gBattleStruct->gimmick.toActivate |= (1 << (battler));
...
static bool32 TryDoGimmicksBeforeMoves(void)
{
    if (!(gHitMarker & HITMARKER_RUN) && gBattleStruct->gimmick.toActivate)
    {
        ...
        for (i = 0; i < gBattlersCount; i++)
        {
            ...
            if ((gBattleStruct->gimmick.toActivate & (1 << (order[i]))) && !(gProtectStructs[order[i]].noValidMoves))
            {
...
static bool32 TryDoMoveEffectsBeforeMoves(void)
{
    if (!(gHitMarker & HITMARKER_RUN))
    {
        ...
        for (i = 0; i < gBattlersCount; i++)
        {
            if (!(gBattleStruct->focusPunchBattlers & (1 << (battlers[i])))
            ...
            {
                gBattleStruct->focusPunchBattlers |= (1 << (battlers[i]));
...
static void HandleEndTurn_FinishBattle(void)
{
    ...
    if (gCurrentActionFuncId == B_ACTION_TRY_FINISH || gCurrentActionFuncId == B_ACTION_FINISHED)
    {
        ...
        for (i = 0; i < PARTY_SIZE; i++)
        {
            ...
            if ((gBattleStruct->appearedInBattle & (1 << (i))) && GetMonData(&gPlayerParty[i], MON_DATA_HP, NULL) != 0)
