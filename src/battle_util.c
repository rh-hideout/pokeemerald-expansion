...
static bool32 IsNonVolatileStatusBlocked(u32 battlerDef, enum Ability abilityDef, enum Ability abilityAffected, const u8 *battleScript, enum ResultOption option);
...
static enum MoveCanceler CancelerTargetFailures(struct BattleContext *ctx)
{
    ...
    while (gBattleStruct->eventState.atkCancelerBattler < gBattlersCount)
    {
        ...
        // No Guard needs to be handled
        if (!BreaksThroughSemiInvulnerablity(ctx->battlerAtk, ctx->battlerDef, ctx->abilityAtk, ctx->abilityDef, ctx->move)
         && gBattleStruct->skyDropTargets[ctx->battlerDef] == SKY_DROP_NO_TARGET) // Maybe there is a more elegant solution to this but without the current battler held by Sky Drop will fail it's move and cancel Sky Drop
        {
            gBattleStruct->moveResultFlags[ctx->battlerDef] |= MOVE_RESULT_FAILED;
...
static void SetPossibleNewSmartTarget(u32 move)
{
    ...
    if (gBattleStruct->moveResultFlags[gBattlerTarget] & MOVE_RESULT_NO_EFFECT
     && !(gBattleStruct->moveResultFlags[partner] & MOVE_RESULT_NO_EFFECT))
...
static enum MoveCanceler CancelerMultihitMoves(struct BattleContext *ctx)
{
    ...
    if (gBattleStruct->moveResultFlags[gBattlerTarget] & MOVE_RESULT_NO_EFFECT) // Dragon Darts can still hit partner
    {
        gMultiHitCounter = 0;
    }
    ...
    else if (GetMoveStrikeCount(ctx->move) > 1)
    {
        ...
        else
        {
            ...
            if (GetBattlerMoveTargetType(ctx->battlerAtk, ctx->move) == TARGET_SMART
             ...
             && IsBattlerImmuneToMove(ctx->battlerDef))
...
static bool32 DoesMoveDoHalfDamage(struct BattleContext *ctx)
{
    ...
    if (!IsZMove(ctx->move) && !IsMaxMove(ctx->move))
        return FALSE;
    return GetProtectType(gProtectStructs[ctx->battlerDef].protected) == PROTECT_TYPE_SINGLE
...
bool32 IsBattlerImmuneToMove(u32 battler)
{
...
bool32 CanMoveSkipAccuracyCalc(u32 battlerAtk, u32 battlerDef, enum Ability abilityAtk, enum Ability abilityDef, u32 move, enum ResultOption option)
{
    bool32 effect = FALSE;
...
bool32 BreaksThroughSemiInvulnerablity(u32 battlerAtk, u32 battlerDef, enum Ability abilityAtk, enum Ability abilityDef, u32 move)
{
