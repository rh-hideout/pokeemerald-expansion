...
static void CancellerConfused(u32 *effect)
{
    ...
    if (gBattleMons[gBattlerAttacker].volatiles.confusionTurns > 0)
    {
...
{
    if (gDisableStructs[battler].boosterEnergyActivates || gBattleMons[battler].volatiles.transformed)
        return ITEM_NO_EFFECT;
...
static inline void MulByTypeEffectiveness(uq4_12_t *modifier, u32 move, u32 moveType, u32 battlerDef, u32 defAbility, u32 defType, u32 battlerAtk, bool32 recordAbilities)
{
    ...
    else if ((moveType == TYPE_FIGHTING || moveType == TYPE_NORMAL) && defType == TYPE_GHOST && gBattleMons[battlerDef].volatiles.foresight && mod == UQ_4_12(0.0))
    {
        mod = UQ_4_12(1.0);
    }
