...
static inline u32 CalcMoveBasePower(struct DamageContext *ctx)
{
    ...
    case EFFECT_ELECTRO_BALL:
        speed = GetBattlerTotalSpeedStat(battlerAtk, ctx->abilities[battlerAtk], ctx->holdEffects[battlerAtk], ctx->weather);
        speed = speed / GetBattlerTotalSpeedStat(battlerDef, ctx->abilities[battlerDef], ctx->holdEffects[battlerDef], ctx->weather);
    ...
    case EFFECT_GYRO_BALL:
        {
            u32 attackerSpeed = GetBattlerTotalSpeedStat(battlerAtk, ctx->abilities[battlerAtk], ctx->holdEffects[battlerAtk], ctx->weather);
            if (attackerSpeed == 0)
            {
                basePower = 1;
            }
            ...
            {
                basePower = ((25 * GetBattlerTotalSpeedStat(battlerDef, ctx->abilities[ctx->battlerDef], ctx->holdEffects[ctx->battlerDef], ctx->weather)) / attackerSpeed) + 1;
                if (basePower > 150)
                    basePower = 150;
            }
            break;
...
void SortBattlersBySpeed(enum BattlerId *battlers, bool32 slowToFast)
{
    ...
    for (i = 0; i < gBattlersCount; i++)
    {
        ...
        speeds[i] = GetBattlerTotalSpeedStat(battler, GetBattlerAbility(battler), GetBattlerHoldEffect(battler), GetWeather());
    }
...
bool32 CanMoveSkipAccuracyCalc(struct BattleCalcValues *cv, u32 weather, enum ResultOption option)
{
    ...
    else if (cv->abilities[battlerDef] == ABILITY_NO_GUARD && !IsSkyDropInvolved(battlerDef, moveEffect))
    {
        effect = TRUE;
        ability = ABILITY_NO_GUARD;
    }
    ...
    if (!effect && weather != B_WEATHER_NONE)
    {
        if (MoveAlwaysHitsInRain(move) && IsBattlerWeatherAffected(cv->holdEffects[battlerDef], weather, B_WEATHER_RAIN)) // Check mega sol interaction then update to GetAttackerWeather.
            effect = TRUE;
        else if ((weather & B_WEATHER_ICY_ANY) && MoveAlwaysHitsInHailSnow(move))
            effect = TRUE;
...
u32 GetTotalAccuracy(struct BattleCalcValues *cv, u32 weather)
{
    ...
    if (MoveHas50AccuracyInSun(move) && IsBattlerWeatherAffected(cv->holdEffects[battlerDef], weather, B_WEATHER_SUN))
        moveAcc = 50;
    // Check Wonder Skin.
    if (cv->abilities[battlerDef] == ABILITY_WONDER_SKIN && IsBattleMoveStatus(move) && moveAcc > 50)
        moveAcc = 50;
    ...
    case ABILITY_SAND_VEIL:
        if (GetAttackerWeather(cv->holdEffects[battlerAtk], cv->abilities[battlerAtk], weather) & B_WEATHER_SANDSTORM)
            calc = (calc * 80) / 100; // 1.2 sand veil loss
        break;
    case ABILITY_SNOW_CLOAK:
        if (GetAttackerWeather(cv->holdEffects[battlerAtk], cv->abilities[battlerAtk], weather) & B_WEATHER_ICY_ANY)
    ...
    switch (cv->abilities[BATTLE_PARTNER(battlerAtk)])
    {
    case ABILITY_VICTORY_STAR:
    ...
    {
        // TODO: Is this true?
        if (cv->abilities[battlerAtk] == ABILITY_RIPEN)
...
bool32 DoesMoveMissTarget(struct BattleCalcValues *cv)
{
    ...
    return !RandomPercentage(RNG_ACCURACY, GetTotalAccuracy(cv, weather));
}
