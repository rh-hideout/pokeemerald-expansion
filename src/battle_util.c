...
void HandleAction_UseMove(void)
{
    ...
    if (gBattleStruct->absentBattlerFlags & (1 << (gBattlerAttacker)) || !IsBattlerAlive(gBattlerAttacker))
    {
...
{
    if (gBattleTypeFlags & BATTLE_TYPE_LINK)
        return (gBattleControllerExecFlags & ((1 << (battler)) << 0x1C)) != 0;
    else
...
{
    if (gBattleTypeFlags & BATTLE_TYPE_LINK)
        gBattleControllerExecFlags |= (1 << (battler)) << (32 - MAX_BATTLERS_COUNT);
    else
...
void MarkBattlerReceivedLinkData(u32 battler)
{
    ...
    for (i = 0; i < GetLinkPlayerCount(); i++)
        gBattleControllerExecFlags |= (1 << (battler)) << (i << 2);
...
void UpdateSentPokesToOpponentValue(u32 battler)
{
    ...
    else
    {
        ...
        for (i = 1; i < gBattlersCount; i++)
            gSentPokesToOpponent[(i & BIT_FLANK) >> 1] |= (1 << (gBattlerPartyIndexes[battler]));
    }
}
...
bool32 IsBelchPreventingMove(u32 battler, u32 move)
{
    ...
    return !(gBattleStruct->ateBerry[battler & BIT_SIDE] & (1 << (gBattlerPartyIndexes[battler])));
}
...
u32 AbilityBattleEffects(u32 caseID, u32 battler, u32 ability, u32 special, u32 moveArg)
{
    ...
    case ABILITYEFFECT_ON_SWITCHIN:
        ...
        case ABILITY_SUPERSWEET_SYRUP:
            if (!gSpecialStatuses[battler].switchInAbilityDone
                    && !(gBattleStruct->supersweetSyrup[GetBattlerSide(battler)] & (1 << (gBattlerPartyIndexes[battler]))))
            {
                gBattlerAttacker = battler;
                gSpecialStatuses[battler].switchInAbilityDone = TRUE;
                gBattleStruct->supersweetSyrup[GetBattlerSide(battler)] |= (1 << (gBattlerPartyIndexes[battler]));
        ...
        case ABILITY_INTREPID_SWORD:
            if (!gSpecialStatuses[battler].switchInAbilityDone && CompareStat(battler, STAT_ATK, MAX_STAT_STAGE, CMP_LESS_THAN)
                 && !(gBattleStruct->intrepidSwordBoost[GetBattlerSide(battler)] & (1 << (gBattlerPartyIndexes[battler]))))
            {
        ...
        case ABILITY_DAUNTLESS_SHIELD:
            if (!gSpecialStatuses[battler].switchInAbilityDone && CompareStat(battler, STAT_DEF, MAX_STAT_STAGE, CMP_LESS_THAN)
                 && !(gBattleStruct->dauntlessShieldBoost[GetBattlerSide(battler)] & (1 << (gBattlerPartyIndexes[battler]))))
            {
                gBattleScripting.savedBattler = gBattlerAttacker;
                gBattlerAttacker = battler;
        ...
        case ABILITY_ZERO_TO_HERO:
            ...
            if (!gSpecialStatuses[battler].switchInAbilityDone
             ...
             && !(gBattleStruct->transformZeroToHero[side] & (1 << (gBattlerPartyIndexes[battler]))))
            {
                gSpecialStatuses[battler].switchInAbilityDone = TRUE;
                gBattlerAttacker = battler;
                gBattleStruct->transformZeroToHero[side] |= (1 << (gBattlerPartyIndexes[battler]));
    ...
    case ITEMEFFECT_ON_SWITCH_IN:
        if (!gSpecialStatuses[battler].switchInItemDone)
        {
            ...
            case HOLD_EFFECT_BOOSTER_ENERGY:
                if (!(gBattleStruct->boosterEnergyActivates & (1 << (battler)))
                  ...
                  || ((GetBattlerAbility(battler) == ABILITY_QUARK_DRIVE) && !(gFieldStatuses & STATUS_FIELD_ELECTRIC_TERRAIN))))
                {
                    PREPARE_STAT_BUFFER(gBattleTextBuff1, GetHighestStatId(battler));
                    gBattleScripting.battler = battler;
                    gBattleStruct->boosterEnergyActivates |= (1 << (battler));
...
u8 IsMonDisobedient(void)
{
    ...
    if (calc < obedienceLevel)
    {
        calc = CheckMoveLimitations(gBattlerAttacker, (1 << (gCurrMovePos)), MOVE_LIMITATIONS_ALL);
...
bool32 IsBattlerAlive(u32 battler)
{
    if (gBattleMons[battler].hp == 0)
        ...
        return FALSE;
    else if (gAbsentBattlerFlags & (1 << (battler)))
        return FALSE;
    else
        return TRUE;
}
...
u32 GetMoveTargetCount(u32 move, u32 battlerAtk, u32 battlerDef)
{
    ...
    {
    case MOVE_TARGET_BOTH:
        return !(gAbsentBattlerFlags & (1 << (battlerDef)))
             + !(gAbsentBattlerFlags & (1 << (BATTLE_PARTNER(battlerDef))));
    case MOVE_TARGET_FOES_AND_ALLY:
        return !(gAbsentBattlerFlags & (1 << (battlerDef)))
             + !(gAbsentBattlerFlags & (1 << (BATTLE_PARTNER(battlerDef))))
             + !(gAbsentBattlerFlags & (1 << (BATTLE_PARTNER(battlerAtk))));
    case MOVE_TARGET_OPPONENTS_FIELD:
        return 1;
    case MOVE_TARGET_DEPENDS:
    case MOVE_TARGET_SELECTED:
...
static inline u32 CalcMoveBasePowerAfterModifiers(u32 move, u32 battlerAtk, u32 battlerDef, u32 moveType, bool32 updateFlags, u32 atkAbility, u32 defAbility, u32 holdEffectAtk, u32 weather)
{
    ...
    case ABILITY_PROTOSYNTHESIS:
        {
            ...
            if ((weather & B_WEATHER_SUN || gBattleStruct->boosterEnergyActivates & (1 << (battlerAtk)))
    ...
    case ABILITY_QUARK_DRIVE:
        {
            u8 atkHighestStat = GetHighestStatId(battlerAtk);
            if ((gFieldStatuses & STATUS_FIELD_ELECTRIC_TERRAIN || gBattleStruct->boosterEnergyActivates & (1 << (battlerAtk)))
            && ((IS_MOVE_PHYSICAL(move) && atkHighestStat == STAT_ATK) || (IS_MOVE_SPECIAL(move) && atkHighestStat == STAT_SPATK)))
                modifier = uq4_12_multiply(modifier, UQ_4_12(1.3));
        }
        break;
    ...
    case ABILITY_PROTOSYNTHESIS:
        {
            ...
            if ((weather & B_WEATHER_SUN || gBattleStruct->boosterEnergyActivates & (1 << (battlerDef)))
    ...
    case ABILITY_QUARK_DRIVE:
        {
            u8 defHighestStat = GetHighestStatId(battlerDef);
            if ((gFieldStatuses & STATUS_FIELD_ELECTRIC_TERRAIN || gBattleStruct->boosterEnergyActivates & (1 << (battlerDef)))
            && ((IS_MOVE_PHYSICAL(move) && defHighestStat == STAT_DEF) || (IS_MOVE_SPECIAL(move) && defHighestStat == STAT_SPDEF)))
                modifier = uq4_12_multiply(modifier, UQ_4_12(0.7));
        }
        break;
...
static inline void MulByTypeEffectiveness(uq4_12_t *modifier, u32 move, u32 moveType, u32 battlerDef, u32 defType, u32 battlerAtk, bool32 recordAbilities)
{
    ...
    if (gBattleStruct->distortedTypeMatchups & (1 << (battlerDef)) || (gBattleStruct->aiCalcInProgress && ShouldTeraShellDistortTypeMatchups(move, battlerDef)))
    {
        mod = UQ_4_12(0.5);
        if (recordAbilities)
              ...
              )))
    {
        return FALSE;
          ...
          (BATTLE_TYPE_EREADER_TRAINER
           ...
           | BATTLE_TYPE_RECORDED_LINK
           | BATTLE_TYPE_SECRET_BASE))
        && (gWishFutureKnock.knockedOffMons[stealerSide] & (1 << (gBattlerPartyIndexes[battlerStealing]))))
    {
        return FALSE;
    }
