...
u8 DoBattlerEndTurnEffects(void)
{
    ...
    while (gBattleStruct->turnEffectsBattlerId < gBattlersCount && gBattleStruct->turnEffectsTracker <= ENDTURN_BATTLER_COUNT)
    {
        ...
        case ENDTURN_INGRAIN:  // ingrain
            if (((gStatuses3[battler] & STATUS3_ROOTED) || (gStatuses3[battler] & STATUS3_FARADAY_CAGED))
             && !BATTLER_MAX_HP(battler)
             && !(gStatuses3[battler] & STATUS3_HEAL_BLOCK)
             && IsBattlerAlive(battler))
             {
...
u8 AtkCanceller_UnableToUseMove(u32 moveType)
{
    ...
    do
    {
        ...
        case CANCELLER_PIVOTING_STOPSIGN: 
        {
            u32 stopsignBattler = IsAbilityOnField(ABILITY_STOP_SIGN);
            if (stopsignBattler && (gMovesInfo[gCurrentMove].effect == EFFECT_HIT_ESCAPE
                             || gMovesInfo[gCurrentMove].effect == EFFECT_PARTING_SHOT))
            {
                gBattleScripting.battler = stopsignBattler - 1;
...
static inline u32 CalcAttackStat(u32 move, u32 battlerAtk, u32 battlerDef, u32 moveType, bool32 isCrit, bool32 updateFlags, u32 atkAbility, u32 defAbility, u32 holdEffectAtk)
{
    ...
    case ABILITY_SOLAR_POWER:
        ...
        break;
    case ABILITY_SHOWER_POWER:
        if (IS_MOVE_SPECIAL(move) && IsBattlerWeatherAffected(battlerAtk, B_WEATHER_RAIN))
            modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(1.5));
        break;
    ...
    case HOLD_EFFECT_THICK_CLUB:
        if ((atkBaseSpeciesId == SPECIES_BANDICUTE || atkBaseSpeciesId == SPECIES_NAUGHTYCOOT) && IS_MOVE_PHYSICAL(move))
            modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(2.0));
        break;
    ...
    case HOLD_EFFECT_CURSED_FANG:
        if (gBattleMons[battlerAtk].species == SPECIES_WINKHULU && IS_MOVE_PHYSICAL(move))
            modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(2.0));
        break;
    case HOLD_EFFECT_CRIMSON_LENS:
        if (gBattleMons[battlerAtk].species == SPECIES_WINKHULU && IS_MOVE_SPECIAL(move))
            modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(2.0));
        break;
    case HOLD_EFFECT_LIGHT_BALL:
        if (atkBaseSpeciesId == SPECIES_AMPSTAR && (B_LIGHT_BALL_ATTACK_BOOST >= GEN_4 || IS_MOVE_SPECIAL(move)))
            modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(2.0));
        break;
...
{
    if (gMovesInfo[move].effect == (EFFECT_HIT_ENEMY_HEAL_ALLY | EFFECT_HIT_TAKE_UR_MEDS)
