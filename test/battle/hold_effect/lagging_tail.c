...
{
    GIVEN {
        PLAYER(TEST_SPECIES_WOBBUFFET) { Speed(100); Item(ITEM_LAGGING_TAIL); }
        PLAYER(TEST_SPECIES_WYNAUT) { Speed(10); }
        OPPONENT(TEST_SPECIES_WOBBUFFET) { Speed(30); Item(ITEM_BERRY_JUICE); }
        OPPONENT(TEST_SPECIES_WYNAUT) { Speed(20); }
    } WHEN {
