...
AI_SINGLE_BATTLE_TEST("AI can boost against opponent with Encore")
{
    PASSES_RANDOMLY(RISK_ENCORE_CHANCE, 100, RNG_AI_RISK_ENCORE);
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { Moves(MOVE_CELEBRATE, MOVE_ENCORE, MOVE_TACKLE); }
        AI_FLAGS(AI_FLAG_CHECK_BAD_MOVE | AI_FLAG_CHECK_VIABILITY | AI_FLAG_TRY_TO_FAINT | AI_FLAG_OMNISCIENT);
        OPPONENT(SPECIES_WOBBUFFET) { Moves(MOVE_SWORDS_DANCE, MOVE_TACKLE); }
    } WHEN {
