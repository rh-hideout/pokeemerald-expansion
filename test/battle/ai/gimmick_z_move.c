...
{
    enum Ability ability;
    PARAMETRIZE { ability = ABILITY_SPEED_BOOST; }
    PARAMETRIZE { ability = ABILITY_OPPORTUNIST; }

    GIVEN {
        AI_FLAGS(AI_FLAG_CHECK_BAD_MOVE | AI_FLAG_CHECK_VIABILITY | AI_FLAG_TRY_TO_FAINT | AI_FLAG_OMNISCIENT );
        ASSUME(GetMoveType(MOVE_CONVERSION) == TYPE_NORMAL);
        PLAYER(SPECIES_ESPATHRA) { Ability(ability); HP(500); MaxHP(500); SpDefense(200); }
        OPPONENT(SPECIES_PORYGON_Z) { Item(ITEM_NORMALIUM_Z); Ability(ABILITY_ADAPTABILITY); SpAttack(50); Moves(MOVE_THUNDERBOLT, MOVE_CONVERSION); }
    } WHEN {
