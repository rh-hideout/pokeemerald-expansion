...
AI_DOUBLE_BATTLE_TEST("AI won't use a Weather changing move if partner already chose such move")
{
    ...
    GIVEN {
        ...
        PLAYER(TEST_SPECIES_WOBBUFFET);
        OPPONENT(TEST_SPECIES_WOBBUFFET) { Moves(weatherMoveLeft); }
        OPPONENT(TEST_SPECIES_WOBBUFFET) { Moves(MOVE_SCRATCH, weatherMoveRight); }
    } WHEN {
...
AI_DOUBLE_BATTLE_TEST("AI will not use Helping Hand if partner does not have any damage moves")
{
    ...
    GIVEN {
        ...
        PLAYER(TEST_SPECIES_WOBBUFFET);
        PLAYER(TEST_SPECIES_WOBBUFFET);
        OPPONENT(TEST_SPECIES_WOBBUFFET) { Moves(MOVE_HELPING_HAND, MOVE_SCRATCH); }
        OPPONENT(TEST_SPECIES_WOBBUFFET) { Moves(move1, move2, move3, move4); }
    } WHEN {
...
AI_DOUBLE_BATTLE_TEST("AI skips Trick/Bestow when items are missing or target already holds one")
{
    ...
    GIVEN {
        AI_FLAGS(AI_FLAG_CHECK_BAD_MOVE | AI_FLAG_CHECK_VIABILITY | AI_FLAG_TRY_TO_FAINT);
        PLAYER(TEST_SPECIES_WOBBUFFET) { Item(targetItem); }
        PLAYER(TEST_SPECIES_WOBBUFFET);
        OPPONENT(TEST_SPECIES_WOBBUFFET) { Moves(move, MOVE_SCRATCH); Item(atkItem); }
        OPPONENT(TEST_SPECIES_WOBBUFFET) { Moves(MOVE_TACKLE); }
    } WHEN {
...
AI_DOUBLE_BATTLE_TEST("AI skips Trick/Bestow around Sticky Hold")
{
    ...
    GIVEN {
        AI_FLAGS(AI_FLAG_CHECK_BAD_MOVE | AI_FLAG_CHECK_VIABILITY | AI_FLAG_TRY_TO_FAINT);
        PLAYER(TEST_SPECIES_WOBBUFFET) { Ability(targetAbility); Item(targetItem); }
        PLAYER(TEST_SPECIES_WOBBUFFET);
        OPPONENT(TEST_SPECIES_WOBBUFFET) { Ability(atkAbility); Item(atkItem); Moves(move, MOVE_SCRATCH); }
        OPPONENT(TEST_SPECIES_WOBBUFFET) { Moves(MOVE_TACKLE); }
    } WHEN {
        TURN { NOT_EXPECT_MOVE(opponentLeft, move); }
    }
...
AI_DOUBLE_BATTLE_TEST("AI skips Trick/Bestow if the target has a Substitute")
{
    ...
    GIVEN {
        AI_FLAGS(AI_FLAG_CHECK_BAD_MOVE | AI_FLAG_CHECK_VIABILITY | AI_FLAG_TRY_TO_FAINT);
        PLAYER(TEST_SPECIES_WOBBUFFET) { Moves(MOVE_SUBSTITUTE, MOVE_CELEBRATE); Item(targetItem); Speed(20); }
        PLAYER(TEST_SPECIES_WOBBUFFET) { Moves(MOVE_CELEBRATE); Speed(20); }
        OPPONENT(TEST_SPECIES_WOBBUFFET) { Moves(move, MOVE_SCRATCH); Item(atkItem); Speed(1); Attack(1); }
...
AI_DOUBLE_BATTLE_TEST("AI will not use a status move if partner already chose Helping Hand")
{
    ...
    GIVEN {
        ...
        PLAYER(TEST_SPECIES_WOBBUFFET) { Moves(MOVE_CELEBRATE, MOVE_SCRATCH, statusMove, MOVE_WATER_GUN); }
        PLAYER(TEST_SPECIES_WOBBUFFET) { Moves(MOVE_CELEBRATE, MOVE_SCRATCH, statusMove, MOVE_WATER_GUN); }
        OPPONENT(TEST_SPECIES_WOBBUFFET) { Moves(MOVE_HELPING_HAND, MOVE_EXPLOSION); }
        OPPONENT(TEST_SPECIES_WOBBUFFET) { Moves(MOVE_SCRATCH, statusMove, MOVE_WATER_GUN); }
    } WHEN {
...
AI_DOUBLE_BATTLE_TEST("AI won't use the same nondamaging move as its partner for no reason")
{
    ...
    GIVEN {
        ...
        PLAYER(TEST_SPECIES_WOBBUFFET);
        PLAYER(TEST_SPECIES_WOBBUFFET);
        OPPONENT(TEST_SPECIES_WOBBUFFET) { Moves(move, MOVE_TACKLE); Status1(STATUS1_BURN); }
        OPPONENT(TEST_SPECIES_WOBBUFFET) { Moves(move, MOVE_TACKLE); }
        OPPONENT(TEST_SPECIES_WOBBUFFET) { Moves(move, MOVE_TACKLE); }
        OPPONENT(TEST_SPECIES_WOBBUFFET) { Moves(move, MOVE_TACKLE); }
    } WHEN {
...
AI_DOUBLE_BATTLE_TEST("Heal Bell and Jungle Healing skip curing a partner that benefits from burn")
{
    ...
    GIVEN {
        ...
        AI_FLAGS(AI_FLAG_CHECK_BAD_MOVE | AI_FLAG_CHECK_VIABILITY | AI_FLAG_TRY_TO_FAINT);
        PLAYER(TEST_SPECIES_WOBBUFFET) { Moves(MOVE_MEMENTO); Speed(1); }
        PLAYER(TEST_SPECIES_WOBBUFFET) { Moves(MOVE_MEMENTO); Speed(1); }
        OPPONENT(TEST_SPECIES_WOBBUFFET) { Moves(move, MOVE_SCRATCH); Speed(20); }
        OPPONENT(TEST_SPECIES_WOBBUFFET) { Ability(ABILITY_GUTS); Moves(MOVE_TACKLE); Status1(STATUS1_BURN); MaxHP(200); HP(200); Speed(10); }
    } WHEN {
        TURN {
...
AI_DOUBLE_BATTLE_TEST("AI will choose Earthquake if partner is not alive")
{
    ...
    GIVEN {
        ...
        PLAYER(TEST_SPECIES_WOBBUFFET);
        PLAYER(TEST_SPECIES_WOBBUFFET);
        OPPONENT(TEST_SPECIES_WOBBUFFET) { Moves(MOVE_EARTHQUAKE, MOVE_SCRATCH); }
...
AI_DOUBLE_BATTLE_TEST("AI will choose Earthquake if it kills one opposing mon and a partner it believes is about to die")
{
    ...
    GIVEN {
        ASSUME(GetMoveTarget(MOVE_EARTHQUAKE) == MOVE_TARGET_FOES_AND_ALLY);
        AI_FLAGS(AI_FLAG_CHECK_BAD_MOVE | AI_FLAG_CHECK_VIABILITY | AI_FLAG_TRY_TO_FAINT | AI_FLAG_OMNISCIENT);
        PLAYER(TEST_SPECIES_WOBBUFFET) { Moves(MOVE_CELEBRATE, MOVE_SCRATCH); }
        PLAYER(TEST_SPECIES_WOBBUFFET) { HP(1); Moves(MOVE_CELEBRATE, MOVE_SCRATCH); }
        OPPONENT(TEST_SPECIES_WOBBUFFET) { Moves(MOVE_EARTHQUAKE, MOVE_SCRATCH); }
...
AI_DOUBLE_BATTLE_TEST("AI will trigger its ally's Weakness Policy")
{
    ...
    GIVEN {
        ...
        PLAYER(TEST_SPECIES_WOBBUFFET);
        PLAYER(TEST_SPECIES_WOBBUFFET);
        OPPONENT(TEST_SPECIES_WOBBUFFET) { Moves(MOVE_EARTHQUAKE, MOVE_STOMPING_TANTRUM); }
...
AI_DOUBLE_BATTLE_TEST("AI treats an ally's redirection ability appropriately")
{
    ...
    GIVEN {
        ...
        PLAYER(TEST_SPECIES_WOBBUFFET);
        PLAYER(TEST_SPECIES_WOBBUFFET);
        OPPONENT(TEST_SPECIES_WOBBUFFET) { Moves(move, MOVE_HEADBUTT); }
...
AI_DOUBLE_BATTLE_TEST("AI sets up weather for its ally")
{
    ...
    GIVEN {
        ...
        OPPONENT(TEST_SPECIES_WOBBUFFET) { Item(ITEM_SAFETY_GOGGLES); Moves(weatherTrigger, MOVE_EARTH_POWER); }
    } WHEN {
...
AI_DOUBLE_BATTLE_TEST("AI sets up terrain for its ally")
{
    ...
    GIVEN {
        ...
        PLAYER(TEST_SPECIES_WOBBUFFET);
        PLAYER(TEST_SPECIES_WOBBUFFET);
        OPPONENT(TEST_SPECIES_WOBBUFFET) { Moves(goodTerrain, badTerrain, MOVE_RETURN, MOVE_TAUNT); }
        OPPONENT(TEST_SPECIES_WOBBUFFET) { Moves(terrainTrigger, MOVE_EARTH_POWER);  }
    } WHEN {
...
AI_DOUBLE_BATTLE_TEST("AI uses Trick Room intelligently")
{
    ...
    GIVEN {
        ...
        AI_FLAGS(AI_FLAG_CHECK_BAD_MOVE | AI_FLAG_TRY_TO_FAINT | AI_FLAG_CHECK_VIABILITY | AI_FLAG_DOUBLE_BATTLE);
        PLAYER(TEST_SPECIES_WOBBUFFET) { Speed(4); }
        PLAYER(TEST_SPECIES_WOBBUFFET) { Speed(speed); }
...
AI_DOUBLE_BATTLE_TEST("AI uses Trick Room with both battlers on the turn it expires in line with the double Trick Room config")
{
    ...
    GIVEN {
        ...
        PLAYER(TEST_SPECIES_WOBBUFFET) { Speed(4); }
        PLAYER(TEST_SPECIES_WOBBUFFET) { Speed(3); }
        OPPONENT(TEST_SPECIES_WYNAUT) { Moves(MOVE_TRICK_ROOM, MOVE_PSYCHIC); Speed(2); }
        OPPONENT(TEST_SPECIES_WOBBUFFET) { Moves(MOVE_TRICK_ROOM, MOVE_PSYCHIC); Speed(1); }
    } WHEN {
...
AI_DOUBLE_BATTLE_TEST("AI uses Helping Hand if it's about to die")
{
    ...
    GIVEN {
        ...
        PLAYER(TEST_SPECIES_WOBBUFFET) { Moves(MOVE_TACKLE, MOVE_CELEBRATE); }
        PLAYER(TEST_SPECIES_WOBBUFFET) { Moves(MOVE_TACKLE, MOVE_CELEBRATE); }
        OPPONENT(TEST_SPECIES_WOBBUFFET) { HP(hp); Moves(MOVE_HELPING_HAND, MOVE_MUDDY_WATER); }
        OPPONENT(TEST_SPECIES_WOBBUFFET) { Moves(MOVE_MUDDY_WATER); }
    } WHEN {
    if (hp == 1)
        TURN { EXPECT_MOVE(opponentLeft, MOVE_HELPING_HAND); }
    else
...
AI_DOUBLE_BATTLE_TEST("AI uses Helping Hand if the ally does notably more damage")
{
    ...
    GIVEN {
        ...
        PLAYER(TEST_SPECIES_WOBBUFFET) { Moves(MOVE_TACKLE, MOVE_CELEBRATE); }
        PLAYER(TEST_SPECIES_WOBBUFFET) { Moves(MOVE_TACKLE, MOVE_CELEBRATE); }
        OPPONENT(TEST_SPECIES_WOBBUFFET) { Moves(MOVE_HELPING_HAND, MOVE_MUD_SLAP); }
        OPPONENT(TEST_SPECIES_WOBBUFFET) { Moves(MOVE_MUDDY_WATER); }
    } WHEN {
        TURN { EXPECT_MOVE(opponentLeft, MOVE_HELPING_HAND); }
    }
...
AI_DOUBLE_BATTLE_TEST("AI uses Tailwind")
{
    ...
    GIVEN {
        ...
        ASSUME(GetMoveEffect(MOVE_TRICK_ROOM) == EFFECT_TRICK_ROOM);
        AI_FLAGS(AI_FLAG_CHECK_BAD_MOVE | AI_FLAG_TRY_TO_FAINT | AI_FLAG_CHECK_VIABILITY | AI_FLAG_DOUBLE_BATTLE);
        PLAYER(TEST_SPECIES_WOBBUFFET) { Speed(speed1); }
        PLAYER(TEST_SPECIES_WOBBUFFET) { Speed(speed2); }
        OPPONENT(TEST_SPECIES_WOBBUFFET) { Speed(speed3); Moves(MOVE_TAILWIND, MOVE_HEADBUTT); }
        OPPONENT(TEST_SPECIES_WOBBUFFET) { Speed(speed4); Moves(MOVE_TAILWIND, MOVE_HEADBUTT); }
    } WHEN {
        if (speed3 > 10)
...
AI_DOUBLE_BATTLE_TEST("AI uses Guard Split to improve its stats")
{
    ...
    GIVEN {
        ...
        ASSUME(gSpeciesInfo[SPECIES_PHEROMOSA].baseDefense < gSpeciesInfo[TEST_SPECIES_WOBBUFFET].baseDefense);
        ASSUME(gSpeciesInfo[TEST_SPECIES_WOBBUFFET].baseDefense < gSpeciesInfo[SPECIES_SHUCKLE].baseDefense);
        ASSUME(gSpeciesInfo[SPECIES_PHEROMOSA].baseSpDefense < gSpeciesInfo[TEST_SPECIES_WOBBUFFET].baseSpDefense);
        ASSUME(gSpeciesInfo[TEST_SPECIES_WOBBUFFET].baseSpDefense < gSpeciesInfo[SPECIES_SHUCKLE].baseSpDefense);
        AI_FLAGS(AI_FLAG_CHECK_BAD_MOVE | AI_FLAG_TRY_TO_FAINT | AI_FLAG_CHECK_VIABILITY | AI_FLAG_DOUBLE_BATTLE);
        PLAYER(player);
        PLAYER(TEST_SPECIES_WOBBUFFET);
        OPPONENT(TEST_SPECIES_WOBBUFFET) { Moves(MOVE_GUARD_SPLIT, MOVE_NIGHT_SHADE); }
        OPPONENT(opponent);
    } WHEN {
...
AI_DOUBLE_BATTLE_TEST("AI uses Power Split to improve its stats")
{
    ...
    GIVEN {
        ...
        ASSUME(gSpeciesInfo[SPECIES_PHEROMOSA].baseAttack > gSpeciesInfo[TEST_SPECIES_WOBBUFFET].baseAttack);
        ASSUME(gSpeciesInfo[TEST_SPECIES_WOBBUFFET].baseAttack > gSpeciesInfo[SPECIES_SHUCKLE].baseAttack);
        ASSUME(gSpeciesInfo[SPECIES_PHEROMOSA].baseSpAttack > gSpeciesInfo[TEST_SPECIES_WOBBUFFET].baseSpAttack);
        ASSUME(gSpeciesInfo[TEST_SPECIES_WOBBUFFET].baseSpAttack > gSpeciesInfo[SPECIES_SHUCKLE].baseSpAttack);
        AI_FLAGS(AI_FLAG_CHECK_BAD_MOVE | AI_FLAG_TRY_TO_FAINT | AI_FLAG_CHECK_VIABILITY | AI_FLAG_DOUBLE_BATTLE);
        PLAYER(player);
        PLAYER(TEST_SPECIES_WOBBUFFET);
        OPPONENT(TEST_SPECIES_WOBBUFFET) { Moves(MOVE_POWER_SPLIT, MOVE_TACKLE, MOVE_ROUND); }
...
AI_DOUBLE_BATTLE_TEST("AI prefers to Fake Out the opponent vulnerable to flinching.")
{
    ...
    GIVEN {
        ...
        OPPONENT(TEST_SPECIES_WOBBUFFET) { Moves(MOVE_FAKE_OUT, MOVE_BRANCH_POKE, MOVE_ROCK_SMASH); }
