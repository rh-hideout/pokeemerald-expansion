...
AI_DOUBLE_BATTLE_TEST("AI uses Follow Me on first turn")
{
    u64 aiFlags = AI_FLAG_CHECK_BAD_MOVE | AI_FLAG_CHECK_VIABILITY | AI_FLAG_TRY_TO_FAINT;

    PARAMETRIZE;
    PARAMETRIZE { aiFlags |= AI_FLAG_OMNISCIENT | AI_FLAG_SMART_SWITCHING | AI_FLAG_SMART_MON_CHOICES | AI_FLAG_PP_STALL_PREVENTION | AI_FLAG_PREDICTION; }

    GIVEN {
        AI_FLAGS(aiFlags);
        PLAYER(SPECIES_WOBBUFFET) { Moves(MOVE_TACKLE, MOVE_CELEBRATE); }
        PLAYER(SPECIES_WOBBUFFET) { Moves(MOVE_TACKLE, MOVE_CELEBRATE); }
        OPPONENT(SPECIES_WOBBUFFET) { Moves(MOVE_FOLLOW_ME, MOVE_STRENGTH); }
        OPPONENT(SPECIES_WOBBUFFET) { Moves(MOVE_PSYCHIC_TERRAIN, MOVE_EXPANDING_FORCE, MOVE_FOLLOW_ME);  }
    } WHEN {
        TURN { EXPECT_MOVE(opponentLeft, MOVE_FOLLOW_ME); EXPECT_MOVE(opponentRight, MOVE_PSYCHIC_TERRAIN); }
        TURN { EXPECT_MOVE(opponentRight, MOVE_EXPANDING_FORCE); }
    }
}
