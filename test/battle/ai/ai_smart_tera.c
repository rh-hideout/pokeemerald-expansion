...
AI_SINGLE_BATTLE_TEST("AI_FLAG_SMART_TERA: AI will tera if it enables a ko")
{
    GIVEN {
        ...
        PLAYER(TEST_SPECIES_WOBBUFFET) { HP(47); Speed(100); }
        PLAYER(TEST_SPECIES_WOBBUFFET) { Speed(100); }
        OPPONENT(TEST_SPECIES_WOBBUFFET) { Speed(1); Moves(MOVE_AQUA_TAIL, MOVE_SEED_BOMB); TeraType(TYPE_GRASS); }
        OPPONENT(TEST_SPECIES_WOBBUFFET) { HP(1); Speed(100); TeraType(TYPE_FIRE); }
    } WHEN {
...
AI_SINGLE_BATTLE_TEST("AI_FLAG_SMART_TERA: AI will not tera if it gets outsped and ko'd")
{
    GIVEN {
        ...
        PLAYER(TEST_SPECIES_WOBBUFFET) { HP(47); Speed(100); Moves(MOVE_FLAMETHROWER, MOVE_CELEBRATE); }
        PLAYER(TEST_SPECIES_WOBBUFFET) { Speed(100); }
        OPPONENT(TEST_SPECIES_WOBBUFFET) { HP(60); Speed(1); Moves(MOVE_SEED_BOMB); TeraType(TYPE_GRASS); }
        OPPONENT(TEST_SPECIES_WOBBUFFET) { HP(1); Speed(100); TeraType(TYPE_FIRE); }
    } WHEN {
        TURN { }
    } SCENE {
...
AI_SINGLE_BATTLE_TEST("AI_FLAG_SMART_TERA: AI will not tera if it gets ko'd by priority")
{
    GIVEN {
        ASSUME(GetMovePower(MOVE_SEED_BOMB) == 80);
        AI_FLAGS(AI_FLAG_CHECK_BAD_MOVE | AI_FLAG_CHECK_VIABILITY | AI_FLAG_TRY_TO_FAINT | AI_FLAG_SMART_TERA | AI_FLAG_OMNISCIENT );
        PLAYER(TEST_SPECIES_WOBBUFFET) { HP(47); Speed(1); Moves(MOVE_QUICK_ATTACK, MOVE_CELEBRATE); }
        PLAYER(TEST_SPECIES_WOBBUFFET) { Speed(100); }
        OPPONENT(TEST_SPECIES_WOBBUFFET) { HP(1); Speed(100); Moves(MOVE_SEED_BOMB, MOVE_AQUA_TAIL); TeraType(TYPE_GRASS); }
        OPPONENT(TEST_SPECIES_WOBBUFFET) { HP(1); Speed(100); TeraType(TYPE_FIRE); }
    } WHEN {
        TURN {  }
    } SCENE {
...
AI_SINGLE_BATTLE_TEST("AI_FLAG_SMART_TERA: AI might tera if it gets saved from a ko")
{
    ...
    GIVEN {
        ASSUME(GetMovePower(MOVE_SEED_BOMB) == 80);
        AI_FLAGS(AI_FLAG_CHECK_BAD_MOVE | AI_FLAG_CHECK_VIABILITY | AI_FLAG_TRY_TO_FAINT | AI_FLAG_SMART_TERA | AI_FLAG_OMNISCIENT );
        PLAYER(TEST_SPECIES_WOBBUFFET) { HP(47); Speed(100); Moves(MOVE_SEED_BOMB); }
        PLAYER(TEST_SPECIES_WOBBUFFET) { Speed(100); }
        OPPONENT(TEST_SPECIES_WOBBUFFET) { Speed(100); HP(30); Moves(MOVE_SEED_BOMB); TeraType(TYPE_FIRE); }
        OPPONENT(TEST_SPECIES_WOBBUFFET) { Speed(100); TeraType(TYPE_FIRE); }
    } WHEN {
