...
AI_SINGLE_BATTLE_TEST("AI uses Dynamax")
{
    GIVEN {
        ...
        OPPONENT(TEST_SPECIES_WOBBUFFET) { Moves(MOVE_SCRATCH); DynamaxLevel(10); }
    } WHEN {
...
AI_SINGLE_BATTLE_TEST("AI uses Dynamax -- Max Moves are scored based on max move effects, not base effects")
{
    ...
    GIVEN {
        AI_FLAGS(AI_FLAG_CHECK_BAD_MOVE | AI_FLAG_CHECK_VIABILITY | AI_FLAG_TRY_TO_FAINT | AI_FLAG_OMNISCIENT );
        PLAYER(TEST_SPECIES_WOBBUFFET);
        OPPONENT(TEST_SPECIES_WOBBUFFET) { Moves(MOVE_SCRATCH, MOVE_EXPLOSION); DynamaxLevel(10); }
    } WHEN {
...
AI_SINGLE_BATTLE_TEST("AI uses Dynamax -- AI does not dynamax before using a utility move")
{
    ...
    GIVEN {
        AI_FLAGS(AI_FLAG_CHECK_BAD_MOVE | AI_FLAG_CHECK_VIABILITY | AI_FLAG_TRY_TO_FAINT | AI_FLAG_OMNISCIENT );
        PLAYER(TEST_SPECIES_WOBBUFFET);
        OPPONENT(TEST_SPECIES_WOBBUFFET) { Moves(MOVE_FAKE_OUT); DynamaxLevel(10); }
    } WHEN {
...
AI_TWO_VS_ONE_BATTLE_TEST("AI only Dynamaxes once per trainer in 2v1 multi battles")
{
    GIVEN {
        AI_FLAGS(AI_FLAG_CHECK_BAD_MOVE | AI_FLAG_CHECK_VIABILITY | AI_FLAG_TRY_TO_FAINT | AI_FLAG_OMNISCIENT);
        MULTI_PLAYER(TEST_SPECIES_WOBBUFFET) { Moves(MOVE_SPLASH); }
        MULTI_PARTNER(TEST_SPECIES_WOBBUFFET) { Moves(MOVE_SPLASH); }
        MULTI_OPPONENT_A(TEST_SPECIES_WOBBUFFET) { Moves(MOVE_SPLASH); DynamaxLevel(10); }
        MULTI_OPPONENT_A(TEST_SPECIES_WOBBUFFET) { Moves(MOVE_SPLASH); DynamaxLevel(10); }
    } WHEN {
        TURN {
