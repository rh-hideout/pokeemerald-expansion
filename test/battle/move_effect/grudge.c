...
SINGLE_BATTLE_TEST("Grudge depletes all PP of the move that fainted the target")
{
    GIVEN {
        ...
        OPPONENT(TEST_SPECIES_WOBBUFFET) { Moves(MOVE_CELEBRATE, MOVE_SCRATCH, MOVE_POUND, MOVE_SURF); };
    } WHEN {
        TURN {
...
SINGLE_BATTLE_TEST("Grudge does not deplete PP of a Z-Move")
{
    GIVEN {
        PLAYER(TEST_SPECIES_WOBBUFFET) { HP(1); }
        PLAYER(TEST_SPECIES_WOBBUFFET);
        OPPONENT(TEST_SPECIES_WOBBUFFET) { Item(ITEM_NORMALIUM_Z); Moves(MOVE_CELEBRATE, MOVE_SCRATCH, MOVE_POUND, MOVE_SURF); };
    } WHEN {
        TURN {
            MOVE(player, MOVE_GRUDGE);
