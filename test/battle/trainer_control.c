...
TEST("CreateNPCTrainerPartyForTrainer generates customized Pokémon")
{
    ...
    u32 currTrainer = 3;
    u8 nickBuffer[20];
    CreateNPCTrainerPartyFromTrainer(testParty, GetTrainerStructFromId(currTrainer), TRUE, BATTLE_TYPE_TRAINER, currTrainer);
...
TEST("Trainer Class Balls apply to the entire party")
{
    ...
    CreateNPCTrainerPartyFromTrainer(testParty, trainer, FALSE, BATTLE_TYPE_TRAINER, currTrainer);
    for(j = 0; j < 6; j++)
    {
...
TEST("Difficulty changes which party is used for enemy trainer if defined for the difficulty (NORMAL)")
{
    ...
    struct Pokemon *testParty = Alloc(6 * sizeof(struct Pokemon));
    u32 currTrainer = 5;
    CreateNPCTrainerPartyFromTrainer(testParty, GetTrainerStructFromId(currTrainer), TRUE, BATTLE_TYPE_TRAINER, 0);
    EXPECT(GetMonData(&testParty[0], MON_DATA_SPECIES) == SPECIES_MEWTWO);
...
TEST("Trainer Party Pool picks according to custom rules")
{
    struct Pokemon *testParty = Alloc(6 * sizeof(struct Pokemon));
    u32 currTrainer = 8;
    CreateNPCTrainerPartyFromTrainer(testParty, GetTrainerStructFromId(currTrainer), TRUE, BATTLE_TYPE_TRAINER | BATTLE_TYPE_DOUBLE, currTrainer);
...
TEST("CreateNPCTrainerPartyForTrainer generates default moves if no moves are specified")
{
    ...
    struct Pokemon *testParty = Alloc(6 * sizeof(struct Pokemon));
    CreateNPCTrainerPartyFromTrainer(testParty, trainer, TRUE, BATTLE_TYPE_TRAINER, currTrainer);
