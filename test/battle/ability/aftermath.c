...
SINGLE_BATTLE_TEST("Aftermath damages the attacker by 1/4th of its max HP if fainted by a contact move")
{
    ...
    GIVEN {
        PLAYER(TEST_SPECIES_WOBBUFFET) { HP(1); Ability(ABILITY_AFTERMATH); };
...
{
    GIVEN {
        PLAYER(TEST_SPECIES_WOBBUFFET) { Ability(ABILITY_POISON_HEAL); };
        OPPONENT(TEST_SPECIES_WOBBUFFET) { HP(1); Ability(ABILITY_AFTERMATH); };
    } WHEN {
...
SINGLE_BATTLE_TEST("Aftermath ability pop-up will be displayed correctly: opponent point of view")
{
    GIVEN {
        PLAYER(TEST_SPECIES_WOBBUFFET) { HP(1); Ability(ABILITY_AFTERMATH); };
        OPPONENT(TEST_SPECIES_WOBBUFFET) { Ability(ABILITY_POISON_HEAL); };
    } WHEN {
