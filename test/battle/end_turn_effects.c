...
DOUBLE_BATTLE_TEST("End Turn Effects: First Event Block is executed correctly (double battle)")
{
    ...
    GIVEN {
        PLAYER(TEST_SPECIES_WYNAUT) { HP(100); Speed(1); }
        PLAYER(SPECIES_EKANS) { HP(100); Ability(ABILITY_SHED_SKIN); Status1(STATUS1_BURN); Speed(2); }
        OPPONENT(TEST_SPECIES_WYNAUT) { HP(100); Item(ITEM_LEFTOVERS); Speed(3); }
        OPPONENT(TEST_SPECIES_WOBBUFFET) { HP(100); Item(ITEM_BLACK_SLUDGE); Speed(4); }
    } WHEN {
...
{
    GIVEN {
        PLAYER(TEST_SPECIES_WYNAUT)      { MaxHP(200); HP(100); Speed(3); }
        ...
        OPPONENT(TEST_SPECIES_WOBBUFFET) { MaxHP(200); HP(100); Speed(4); }
    } WHEN {
        TURN {
...
MULTI_BATTLE_TEST("End Turn Effects: First Event Block is executed correctly (multibattle)")
{
    ...
    GIVEN {
        MULTI_PLAYER(TEST_SPECIES_WYNAUT) { HP(100); Speed(1); }
        MULTI_PARTNER(SPECIES_EKANS) { HP(100); Ability(ABILITY_SHED_SKIN); Status1(STATUS1_BURN); Speed(2); }
        MULTI_OPPONENT_A(TEST_SPECIES_WYNAUT) { HP(100); Item(ITEM_LEFTOVERS); Speed(3); }
        MULTI_OPPONENT_B(TEST_SPECIES_WOBBUFFET) { HP(100); Item(ITEM_BLACK_SLUDGE); Speed(4); }
    } WHEN {
        TURN { MOVE(playerLeft, MOVE_GRASSY_TERRAIN); }
    } SCENE {
        MESSAGE("The opposing Wobbuffet is healed by the grassy terrain!");
...
TWO_VS_ONE_BATTLE_TEST("End Turn Effects: First Event Block is executed correctly (2v1)")
{
    ...
    GIVEN {
        ...
        MULTI_PARTNER(SPECIES_EKANS) { HP(100); Ability(ABILITY_SHED_SKIN); Status1(STATUS1_BURN); Speed(2); }
        MULTI_OPPONENT_A(TEST_SPECIES_WYNAUT) { HP(100); Item(ITEM_LEFTOVERS); Speed(3); }
        MULTI_OPPONENT_A(TEST_SPECIES_WOBBUFFET) { HP(100); Item(ITEM_BLACK_SLUDGE); Speed(4); }
    } WHEN {
        TURN { MOVE(playerLeft, MOVE_GRASSY_TERRAIN); }
    } SCENE {
        MESSAGE("The opposing Wobbuffet is healed by the grassy terrain!");
