...
case "$SCENARIO" in
    boot)
        ...
        ;;
    lab-campfire)
        SCRIPT='expect-probe:map_group,1;expect-probe:map_num,4;command:starter-campfire;expect-probe:spot_id,7;expect-probe:resolved_species,255;expect-probe:spot_state,3;expect-probe:resident_spot_id,7;wait:60;shot:lab_campfire;pass:Campfire starter transition reached'
        ;;
    lab-plant)
        SCRIPT='expect-probe:map_group,1;expect-probe:map_num,4;command:starter-plant;expect-probe:spot_id,7;expect-probe:resolved_species,252;expect-probe:spot_state,3;expect-probe:resident_spot_id,7;wait:60;shot:lab_plant;pass:Plant starter transition reached'
        ;;
    lab-basin)
        SCRIPT='expect-probe:map_group,1;expect-probe:map_num,4;command:starter-basin;expect-probe:spot_id,7;expect-probe:resolved_species,258;expect-probe:spot_state,3;expect-probe:resident_spot_id,7;wait:60;shot:lab_basin;pass:Basin starter transition reached'
        ;;
    starter-recovery)
        SCRIPT='command:starter-campfire;expect-probe:resolved_species,255;command:recover-treecko;expect-probe:spot_id,8;expect-probe:spot_state,3;expect-probe:resolved_species,252;expect-probe:resident_spot_id,8;command:recover-mudkip;expect-probe:spot_id,9;expect-probe:spot_state,3;expect-probe:resolved_species,258;expect-probe:resident_spot_id,9;wait:60;shot:starter_recovery;pass:Both unchosen starters recovered through their authored furnishing chains'
        ;;
    skitty)
