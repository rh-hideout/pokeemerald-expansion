...
def create_tutor_moves_array(tutors: list[str]) -> None:
    """
    Generate gTutorMoves[] if POKEDEX_PLUS_HGSS is enabled.
    """
    # Check if the config is enabled
    with open("./include/config/pokedex_plus_hgss.h", "r") as cfg_pokedex_plus_hgss:
        cfg_pokedex_plus_hgss = cfg_pokedex_plus_hgss.read()
        cfg_defined = TUTOR_ARRAY_ENABLED_PAT.search(cfg_pokedex_plus_hgss)
    ...
    header = dedent("""\
        ...
        // This array is enabled when the HGSS dex is
