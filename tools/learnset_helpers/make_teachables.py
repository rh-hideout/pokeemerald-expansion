...
TUTOR_ARRAY_ENABLED_PAT = re.compile(r"#define\s+TUTOR_MOVES_ARRAY\s+(TRUE|1)\b")
...
def tutor_array_enabled() -> bool:
    with open("./include/config/general.h") as f:
        return bool(TUTOR_ARRAY_ENABLED_PAT.search(f.read()))


def emit_tutor_moves_array(tutors: list[str]) -> None:
    """
    Generate gTutorMoves[] if TUTOR_MOVES_ARRAY is enabled.
    """
    header = dedent("""\
        // DO NOT MODIFY THIS FILE! It is auto-generated by tools/learnset_helpers/make_teachables.py
        // Set the config TUTOR_MOVES_ARRAY in include/config/general.h to TRUE to enable this array!
...
def main():
    ...
    if tutor_array_enabled():
        emit_tutor_moves_array(repo_tutors)
