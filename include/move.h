...
{
    moveId = SanitizeMoveId(moveId);
    assertf(0, "not a terrain flag move: %S", gMovesInfo[moveId].name);
...
static inline u32 GetMoveEffectArg_MoveProperty(u32 moveId)
{
    moveId = SanitizeMoveId(moveId);
    enum BattleMoveEffects effect = gMovesInfo[moveId].effect;
    assertf(effect == EFFECT_FIRST_TURN_ONLY || effect == EFFECT_HEAL_PULSE || effect == EFFECT_MULTI_HIT, "not a move with moveProperty: %S", gMovesInfo[moveId].name);
...
{
    moveId = SanitizeMoveId(moveId);
    switch (GetMoveEffect(moveId))
    {
    case EFFECT_NON_VOLATILE_STATUS:
    case EFFECT_YAWN:
    case EFFECT_DARK_VOID:
        return gMovesInfo[moveId].argument.nonVolatileStatus;
    default:
...
{
    moveId = SanitizeMoveId(moveId);
    assertf(gMovesInfo[moveId].effect == EFFECT_FIXED_PERCENT_DAMAGE, "not a percentage-damage move: %S", gMovesInfo[moveId].name);
    return gMovesInfo[moveId].argument.damagePercentage;
}

static inline u32 GetMoveOverwriteAbility(u32 moveId)
{
    moveId = SanitizeMoveId(moveId);
    assertf(gMovesInfo[moveId].effect == EFFECT_OVERWRITE_ABILITY, "not a move that overwrites abilities: %S", gMovesInfo[moveId].name);
    return gMovesInfo[moveId].argument.overwriteAbility;
}
