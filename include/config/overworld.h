...
#define OW_WILD_ENCOUNTERS_RESTRICT_METATILE            TRUE    // If TRUE, OW Pokémon spawned as overworld wild encounters will stay within tiles with the same encounter metatile behavior as the one it is currently on, if any.
...
#define OW_WILD_ENCOUNTERS_UNRESTRICT_SIGHT             FALSE   // If TRUE, OW Pokémon spawned as overworld wild encounters will ignore all movement restrictions when they can see the player.
...
#define OW_WILD_ENCOUNTERS_SHINY_SPARKLE                FALSE   // If TRUE, a shiny overworld wild encounter will spawn with a sparkle animation and play the shiny sound effect.
...
#define OW_WILD_ENCOUNTERS_PREVENT_SHINY_REPLACEMENT    TRUE    // If TRUE, shiny OW Pokémon objects will not be despawned if OW_WILD_ENCOUNTERS_SPAWN_REPLACEMENT is TRUE.
#define OW_WILD_ENCOUNTERS_PREVENT_FEEBAS_REPLACEMENT   TRUE    // If TRUE, Feebas OW objects spawned from special Feebas fishing spots (when OW_WILD_ENCOUNTERS_FEEBAS is TRUE) will not be despawned if OW_WILD_ENCOUNTERS_SPAWN_REPLACEMENT is TRUE.
