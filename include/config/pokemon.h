...
#define P_MOVE_INHERITANCE             GEN_LATEST                // Prior to Gen 6, eggs would inherit egg moves from tehir father, tm moves from their father, level up moves shared by both parents in that order. In gen 6 and higher, eggs would inherit level up moves shared by both parents, egg moves from theri father, egg moves from their mother in that order.
