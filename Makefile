...
RELEASE      ?= 0
# Some generated files will sometimes not be recreated if deleted, this forces them to be rebuilt
FORCE_GENERATED ?= 1
...
# Default make rule
ifeq ($(FORCE_GENERATED),1)
all: generated_intermediates rom
else
all: rom
endif
...
TEACHABLE_LEARNSETS := $(DATA_SRC_SUBDIR)/pokemon/teachable_learnsets.h
...
.PHONY: all rom agbcc modern compare check debug release generated_intermediates
.PHONY: $(RULES_NO_SCAN)
...
ifneq ( ,$(wildcard $(LEARNSET_HELPERS_BUILD_DIR)/all_learnables.json))
...
$(C_BUILDDIR)/pokemon.o: $(C_SUBDIR)/pokemon.c $(TEACHABLE_LEARNSETS)
...
ifneq ( ,$(wildcard $(LEARNSET_HELPERS_BUILD_DIR)/all_learnables.json))
	mv $(LEARNSET_HELPERS_BUILD_DIR)/all_learnables.json $(ALL_LEARNABLES_JSON)
else
...
$(TEACHABLE_LEARNSETS): $(TEACHABLE_DEPS)
	python3 $(LEARNSET_HELPERS_DIR)/make_teachables.py $(LEARNSET_HELPERS_BUILD_DIR)

generated_intermediates: $(ALL_TUTORS_JSON) $(ALL_TEACHING_TYPES_JSON) $(ALL_LEARNABLES_JSON) $(TEACHABLE_LEARNSETS)
