...
# Use the same C preprocessor and include paths as the build when resolving
# P_LVL_UP_LEARNSETS and its config aliases.
LEVEL_UP_LEARNSETS_CPP_ARGS := $(CPP) $(INCLUDE_CPP_ARGS) -imacros $(INCLUDE_DIRS)/global.h
...
# Explicitly remove the project-owned level-up learnsets header. The next build
...
$(C_BUILDDIR)/pokemon.o: $(C_SUBDIR)/pokemon.c $(DATA_SRC_SUBDIR)/pokemon/teachable_learnsets.h $(LEVEL_UP_LEARNSETS)
...
# This target deliberately has no prerequisites. Make creates it only when it
# is missing, allowing projects to edit it without later builds overwriting it.
$(LEVEL_UP_LEARNSETS):
	python3 $(LEARNSET_HELPERS_DIR)/make_level_up_learnsets.py $(LEVEL_UP_LEARNSETS_CPP_ARGS) -- $(LEARNSET_HELPERS_DATA_DIR) $(DATA_SRC_SUBDIR)/pokemon $@

# Explicitly replace the project-owned header with the selected official data.
